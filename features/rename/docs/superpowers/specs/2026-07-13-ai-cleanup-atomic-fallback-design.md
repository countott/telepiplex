...
rename 优先消费 download 发布的真实 `download_root`、完整 `file_tree`、Prowlarr `release` 证据和已确认 `media_metadata`。旧事件仍可通过 `storage.provider` 重建文件树。

direct magnet 没有 canonical contract 时，rename 先调用 `media.search.resolve_metadata`，由 search 重新查询 Wikipedia、豆瓣、TVDB，并在严格证据不足时调用 AI。成功后进入同一 canonical 流程；失败时保留 legacy TVDB+AI 兼容路径，最终仍不能确认则整批进入未整理。
