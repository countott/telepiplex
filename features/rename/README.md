...
rename 1.0.6 会对旧版搜索合同做防御性规范化：英语作品优先采用来源原名，并从混合中文标题末尾移除已经重复的英文标题，避免生成 `后室 Backrooms (Backrooms)` 一类目录。
...
```bash
python tools/build_feature.py features/rename /tmp/rename-1.0.6.tpx \
