...
from .config_wizard import SyncConfigWizard
from .jobs import PlexJobRepository
from .sync_service import LibrarySyncService, PlexOperationCancelled
...
class SyncFeature:
    def __init__(
        ...
        *,
        ...
        self.config_wizard = SyncConfigWizard(config)
    ...
    def bind_runtime(self, runtime):
        ...
        if self.interrupted_job_ids:
            ...
            # route can receive a replayed Host event.  Reporting the snapshot
            # may wait for Host, but local replay suppression must not.
    ...
    async def command(self, request: dict) -> dict:
        ...
        if not waiting:
            return self._message(LibrarySyncService.format_job_summary(job))
    ...
    async def _ensure_service(self):
        ...
        async with self._service_lock:
            ...
            try:
                ...
                self.service_error = LibrarySyncService._safe_error(exc)
                raise
            return self.service

    def _build_service(self):
        sync_config = self.config.get("plex") or {}
        base_url = str(sync_config.get("base_url") or "").strip()
        token = str(sync_config.get("token") or "").strip()
        ...
        if mcp_config.get("enabled"):
            try:
                ...
                self.service_error = f"MCP isolated: {LibrarySyncService._safe_error(exc)}"
        return service
    ...
    async def _confirm_event_ownership(self, operation):
        ...
        try:
            ...
            operation.update({
                "state": "interrupted",
                "status_text": "Host 已结束协调任务，未开始 Plex 操作。",
                "control": "",
            })
    ...
    async def _complete_batch_operation(self, operation_id, results):
        ...
        if failed:
            return await self._report_operation(
                ...
                status_text=LibrarySyncService.format_job_summary(failed[0]),
                control="",
            )
        return await self._report_operation(
            operation_id,
    ...
    async def _report_operation(self, operation_id, **changes):
        ...
        if not isinstance(response, dict) or response.get("accepted") is not True:
            ...
            raise FeatureError(
                "operation_rejected",
                "Host rejected sync operation ownership",
            )
        return view
    ...
    async def _resume_interrupted(self):
        for job_id in self.interrupted_job_ids:
            ...
            if operation_id:
                ...
                await self.host.report_operation(operation)
