...
sync 1.1.4 不订阅 rename 事件，也不会在 rename 完成后自动扫描 Plex。Plex 扫描与增强只能由用户通过 Telegram 命令或带确认令牌的 MCP 写工具独立发起；rename 的成功或失败不依赖 sync 是否安装、启用或可用。
...
```bash
python tools/build_feature.py features/sync /tmp/sync-1.1.4.tpx \
