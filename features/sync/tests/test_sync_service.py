...
def make_completion():
    ...
    return DownloadPipelineCompletion(
        ...
        terminal_processor="rename.generic_media",
    )
...
def make_media_metadata_completion(mapping_kind):
    ...
    return DownloadPipelineCompletion(
        event=event,
        result=result,
        terminal_processor="rename.media_metadata",
    )
...
class LibrarySyncServiceTest(unittest.TestCase):
    ...
    def test_enqueue_is_idempotent_and_ignores_non_rename_completion(self):
        service = self.make_service()
        completion = make_completion()

        first = service.enqueue_completion(completion)
        second = service.enqueue_completion(completion)
        object.__setattr__(completion, "terminal_processor", "download.unorganized_fallback")
