...
from telepiplex_sync.sync_service import (
    LibrarySyncService,
...
class SyncFeatureRuntimeTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_media_event_accepts_chain_operation_and_reports_all_stages(self):
        ...
        stages = {report["stage"] for report in self.feature.host.reports}
        self.assertTrue({
    ...
    async def test_scan_selection_validates_fresh_list_and_scans_only_that_id(self):
        ...
        self.assertEqual(self.feature.host.reports[-1]["state"], "completed")
        self.assertIn("电影", self.feature.host.reports[-1]["status_text"])
    ...
    async def test_pending_selection_cancel_persists_job_and_discloses_no_rollback(self):
        ...
        service = LibrarySyncService(self.jobs, SimpleNamespace())
    ...
    async def test_plex_history_filters_owner_before_limit(self):
        ...
        self.feature.service = LibrarySyncService(
            self.jobs,
            SimpleNamespace(),
        )
    ...
    async def test_choice_cancel_before_worker_skips_write_and_cancels_job(self):
        ...
        self.feature.service = LibrarySyncService(self.jobs, plex)
        opened = await self.feature.command({
            "command": "sync",
            "args": [str(job["id"])],
            "chat_id": 10,
    ...
    async def test_confirm_selection_checks_cancel_before_any_plex_mutation(self):
        ...
        for kind, candidate in candidates.items():
            with self.subTest(kind=kind):
                ...
                service = LibrarySyncService(self.jobs, plex)
    ...
    async def test_choice_pick_confirms_and_continues_same_operation(self):
        ...
        self.assertEqual(
            self.feature.host.reports[-1]["operation_id"],
            operation_id,
        )
        self.assertEqual(self.feature.host.reports[-1]["state"], "completed")
    ...
    async def test_cancelled_job_stops_after_current_plex_step(self):
        ...
        self.assertEqual(self.feature.host.reports[-1]["state"], "cancelled")
    ...
    async def test_uncoordinated_interrupted_job_requires_explicit_retry(self):
        ...
        feature = SyncFeature(
            config={}, host=FakeHost(), state_path=self.root / "state2",
            repository=self.jobs, service_factory=lambda: self.service,
        )
        runtime = FakeRuntime()
        feature.bind_runtime(runtime)
    ...
    async def test_coordinated_interrupted_job_is_reported_not_auto_resumed(self):
        ...
        host = FakeHost()
        feature = SyncFeature(
            config={}, host=host, state_path=self.root / "state-coordinated",
            repository=self.jobs, service_factory=lambda: self.service,
        )
        runtime = FakeRuntime()
        feature.bind_runtime(runtime)
    ...
    async def test_coordinated_interruption_reports_before_service_initializes(self):
        ...
        self.assertEqual(host.reports[-1]["state"], "interrupted")
        self.assertEqual(host.reports[-1]["operation_id"], "op-broken-service")
        self.assertEqual(self.jobs.get(job["id"])["state"], "interrupted")
    ...
    async def test_pending_event_replay_cannot_restart_interrupted_operation(self):
        ...
        host = FakeHost()
        feature = SyncFeature(
            config={}, host=host, state_path=self.root / "state-replay",
            repository=self.jobs, service_factory=lambda: self.service,
        )
        runtime = FakeRuntime()
        feature.bind_runtime(runtime)
    ...
    async def test_missing_plex_credentials_does_not_break_feature_startup(self):
        ...
        context = SimpleNamespace(
            manifest={"plugin_id": "sync", "version": "1.0.0"},
            token="token",
            host=FakeHost(),
...
class FeatureSourceContractTest(unittest.TestCase):
    ...
    def test_package_and_readme_use_current_version(self):
        ...
        self.assertEqual(package["project"]["version"], "1.0.0")
        self.assertIn("/tmp/sync-1.0.0.tpx", source)
        self.assertNotIn("dist/sync-1.0.0.tpx", source)
...
class PlexManualScanServiceTest(unittest.TestCase):
    def test_scan_libraries_continues_after_failure(self):
        ...
        with tempfile.TemporaryDirectory() as temp:
            ...
            plex = ScanPlex()
            service = LibrarySyncService(jobs, plex)
