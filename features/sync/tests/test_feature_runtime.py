...
class SyncFeatureRuntimeTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_missing_plex_credentials_does_not_break_feature_startup(self):
        ...
        context = SimpleNamespace(
            manifest={"plugin_id": "sync", "version": "2.0.0"},
            token="token",
            host=FakeHost(),
...
class FeatureSourceContractTest(unittest.TestCase):
    ...
    def test_package_and_readme_use_current_version(self):
        ...
        self.assertEqual(package["project"]["version"], "2.1.0")
        self.assertEqual(
            package["project"]["dependencies"][0],
            "telepiplex-plugin-sdk==2.2.0",
        )
        self.assertIn("/tmp/sync-2.1.0.tpx", source)
        self.assertNotIn("dist/sync-2.1.0.tpx", source)
