...
8. `/s` 下载任务必须把已确认契约传给 rename；rename 不重新判断作品身份。
...
rename 不重新搜索媒体实体，不从文件树创建新季集，不覆盖已确认标题。
...
`items`包含本次目标范围内全部已确认、已播出的 TVDB 普通集。该契约只随下载任务流转，rename/Plex 交接完成后释放。

## 19. rename 结构化文件映射

### 19.1 `/s`任务

rename 必须直接使用 confirmed 契约：
...
没有 confirmed 契约时，rename 只构造结构化探针：
...
正确性门禁：movie
下载对象：完整电影
最终放置：rename 根据已确认 placement 整理
```

search 只有在用户选定电影后，关系假设被来源验证且用户确认时，才能写入当前任务 `placement.mapping_kind`。rename 不能下载后自行建立关系。
