...
**Goal:** Make ordinary text searches use an AI-governed, bounded tool loop over Wikipedia, Douban, and TVDB before deterministic candidate verification, while preserving direct-link identity locks, user confirmation, program-generated Prowlarr queries, and the existing rename handoff contract.
...
- Use `/Users/young/.cache/codex-runtimes/codex-primary-runtime/dependencies/python/bin/python3` with `PYTHONPATH=src:/Users/young/Documents/telepiplex/sdk/src`.
...
```bash
PYTHONPATH=src:/Users/young/Documents/telepiplex/sdk/src /Users/young/.cache/codex-runtimes/codex-primary-runtime/dependencies/python/bin/python3 -m unittest tests.test_search_ai_pipeline -v
```
...
```bash
git add src/telepiplex_search/ai.py tests/test_search_ai_pipeline.py
git commit -m "feat(search): add AI tool-call transport"
...
**Files:**
- Create: `src/telepiplex_search/source_tools.py`
...
```bash
PYTHONPATH=src:/Users/young/Documents/telepiplex/sdk/src /Users/young/.cache/codex-runtimes/codex-primary-runtime/dependencies/python/bin/python3 -m unittest tests.test_source_tools tests.test_douban_adapter tests.test_tvdb_adapter -v
```
...
```bash
git add src/telepiplex_search/source_tools.py src/telepiplex_search/adapters/douban.py src/telepiplex_search/adapters/tvdb.py src/telepiplex_search/service.py tests/test_source_tools.py tests/test_douban_adapter.py tests/test_tvdb_adapter.py
git commit -m "feat(search): add bounded source tool gateway"
git push origin feature/search
```
...
**Files:**
- Create: `src/telepiplex_search/source_orchestrator.py`
- Create: `src/telepiplex_search/evidence_verifier.py`
...
```bash
git add src/telepiplex_search/source_orchestrator.py src/telepiplex_search/evidence_verifier.py src/telepiplex_search/entity_graph.py tests/test_source_orchestrator.py tests/test_evidence_verifier.py tests/test_entity_graph.py
git commit -m "feat(search): orchestrate AI source queries"
git push origin feature/search
```
...
```bash
git add src/telepiplex_search/planner.py src/telepiplex_search/service.py tests/test_ranked_planner.py tests/test_search_planner_service.py tests/test_feature_service.py
git commit -m "feat(search): make source orchestration AI first"
git push origin feature/search
```

### Task 5: Publish configuration, documentation, and version 1.0.0
...
- Produces Feature version `1.0.0`.
...
Document AI-first ordinary text, fixed first round, two autonomous targeted rounds, source degradation, TVDB credential isolation, deterministic direct links/fallback, and unchanged rename handoff. Bump manifest/project/build example to `1.0.0`.

- [ ] **Step 5: Verify GREEN, build artifact, commit, and push**

Run focused tests, full suite, compile, schema validation, build `dist/search-1.0.0.tpx`, inspect the archive, then:
...
git commit -m "chore(search): prepare 1.0.0"
git push origin feature/search
```

### Task 6: Verify the search-confirmation-rename boundary and final remote state

**Files:**
- Inspect: `src/telepiplex_search/service.py`
- Inspect: `/Users/young/Documents/telepiplex/features/download/src`
- Inspect: `/Users/young/Documents/telepiplex/features/rename/src`
- Test: `tests/test_feature_service.py`
- Test: `/Users/young/Documents/telepiplex/features/rename/tests`
...
- [ ] **Step 2: Audit Open115 pass-through and rename merge order**

Trace the actual code from `download.provider.submit` through `download.completed` to rename. If the confirmed metadata is already preserved, make no cross-branch code change. If a defect is proven, first add a failing test in the owning worktree, then make the smallest branch-local fix and push only that affected feature branch.

- [ ] **Step 3: Run composed verification**

Run search full unittest/pytest, compile, `pip check`, schema validation, artifact inspection, and whitespace checks. Run the rename contract tests with Python 3.12 and the Host SDK on `PYTHONPATH`.
...
```bash
git rev-list --left-right --count feature/search...origin/feature/search
```

Expected: `0 0`. If rename or download changed, require the same exact synchronization for that branch.

- [ ] **Step 5: Final report**

Report commit hashes, remote branches, test counts, artifact result, and the concrete search → confirmation → download → rename evidence. Explicitly state that release-gate grammar remains a separate follow-up.
