...
**Goal:** Build a bounded `/s` pipeline that sends simple canonical queries to Prowlarr, rejects wrong media identities and scopes before quality scoring, restores detailed Indexer/score output, and hands rename a structured identity probe instead of a concatenated file tree.
...
- `/s` downloads carry confirmed `media_metadata`; rename cannot re-resolve their identity.
...
**Files:**
- Create: `src/telepiplex_search/query_normalization.py`
- Modify: `src/telepiplex_search/input_contract.py`
- Modify: `src/telepiplex_search/search_resolution.py`
- Modify: `src/telepiplex_search/direct_link.py`
...
```bash
git add src/telepiplex_search/query_normalization.py src/telepiplex_search/input_contract.py src/telepiplex_search/search_resolution.py src/telepiplex_search/direct_link.py src/telepiplex_search/service.py tests/test_input_contract.py tests/test_direct_link.py
git commit -m "feat(search): bound user query grammar"
```

### Task 2: Centralize Canonical Prowlarr Query Construction

**Files:**
- Create: `src/telepiplex_search/prowlarr_query.py`
- Modify: `src/telepiplex_search/planner.py`
- Modify: `src/telepiplex_search/search_resolution.py`
- Modify: `src/telepiplex_search/series_scope.py`
- Modify: `src/telepiplex_search/direct_link.py`
- Modify: `src/telepiplex_search/service.py`
...
```bash
/Users/young/.cache/codex-runtimes/codex-primary-runtime/dependencies/python/bin/python3 -m unittest tests.test_media_search_utils tests.test_series_scope tests.test_direct_link tests.test_feature_service.SearchFeatureTest.test_confirmed_plan_searches_prowlarr_in_english_only tests.test_feature_service.SearchFeatureTest.test_bare_series_requires_scope_before_prowlarr -v
```
...
- `DirectEntity.query`;
- `apply_series_scope`;
- `SearchFeature._english_prowlarr_query`.
...
```bash
git add src/telepiplex_search/prowlarr_query.py src/telepiplex_search/planner.py src/telepiplex_search/search_resolution.py src/telepiplex_search/series_scope.py src/telepiplex_search/direct_link.py src/telepiplex_search/service.py tests/test_media_search_utils.py tests/test_series_scope.py tests/test_direct_link.py tests/test_feature_service.py
git commit -m "refactor(search): centralize Prowlarr queries"
```
...
- Modify: `src/telepiplex_search/candidate_score.py`
- Modify: `src/telepiplex_search/planner.py`
...
```bash
git add src/telepiplex_search/ai.py src/telepiplex_search/candidate_score.py src/telepiplex_search/planner.py tests/test_search_planner_service.py tests/test_ranked_planner.py
git commit -m "feat(search): score candidates from verified facts"
```

### Task 4: Add the Independent Release Correctness Gate

**Files:**
- Create: `src/telepiplex_search/release_gate.py`
- Create: `tests/test_release_gate.py`
- Modify: `src/telepiplex_search/release_score.py`
...
```bash
git add src/telepiplex_search/release_gate.py src/telepiplex_search/release_score.py tests/test_release_gate.py
git commit -m "feat(search): gate release identity and scope"
```
...
**Files:**
- Modify: `src/telepiplex_search/release_score.py`
- Create: `src/telepiplex_search/release_report.py`
...
```bash
git add src/telepiplex_search/release_score.py src/telepiplex_search/release_report.py tests/test_media_search_utils.py tests/test_release_report.py
git commit -m "feat(search): explain release quality scores"
```
...
**Files:**
- Modify: `src/telepiplex_search/service.py`
- Modify: `src/telepiplex_search/adapters/prowlarr.py`
...
**Interfaces:**
- `SearchFeature` accepts injectable `indexer_summary`.
...
```bash
git add src/telepiplex_search/service.py src/telepiplex_search/adapters/prowlarr.py config.default.yaml config.schema.json tests/test_feature_service.py tests/test_config_schema_contract.py
git commit -m "feat(search): enforce exact release cohorts"
```

### Task 7: Replace Renaming File-Tree Queries with Structured Probes

**Worktree:** `/Users/young/Documents/telepiplex/features/rename`

**Files:**
- Create: `src/telepiplex_rename/content_probe.py`
- Modify: `src/telepiplex_rename/service.py`
- Modify: `tests/test_feature_processor.py`
- Modify: `/Users/young/Documents/telepiplex/features/search/src/telepiplex_search/service.py`
- Modify: `/Users/young/Documents/telepiplex/features/search/tests/test_feature_service.py`
...
- search noninteractive resolution still requires exactly one hard-gate candidate.

- [ ] **Step 1: Write failing probe tests in rename**
...
async def test_direct_download_sends_structured_probe_not_file_tree_sentence(self):
    ...
    self.assertEqual(host.metadata_payload["probe"]["content_shape"], "movie")
    self.assertNotIn("|", host.metadata_payload["query"])
```

- [ ] **Step 2: Run focused rename tests and verify failure**

```bash
cd /Users/young/Documents/telepiplex/features/rename
/Users/young/.cache/codex-runtimes/codex-primary-runtime/dependencies/python/bin/python3 -m unittest tests.test_feature_processor.RenameFeatureTest.test_direct_magnet_requeries_media_search_with_release_and_file_tree -v
```
...
In search `metadata_capability()`, accept and log the optional structured `probe` without appending it to the query. Continue only when exactly one candidate is selectable; otherwise raise `metadata_unresolved`.
...
/Users/young/.cache/codex-runtimes/codex-primary-runtime/dependencies/python/bin/python3 -m unittest tests.test_feature_service.SearchFeatureTest.test_metadata_capability_resolves_once_without_registry -v
```
...
```bash
git add src/telepiplex_rename/content_probe.py src/telepiplex_rename/service.py tests/test_feature_processor.py
git commit -m "fix(rename): structure metadata probes"
```
...
git commit -m "feat(search): accept structured metadata probes"
```
...
- Modify: `README.md`
- Modify: `tests/test_feature_service.py`
- Modify in rename worktree: `manifest.yaml`
- Modify in rename worktree: `pyproject.toml`
- Modify in rename worktree: `README.md`
- Modify in rename worktree: `tests/test_feature_processor.py`

**Interfaces:**
- Produces: search `1.4.0`.
- Produces: rename `1.3.0`.
- Produces buildable `.tpx` archives.

- [ ] **Step 1: Run search focused suites**
...
- [ ] **Step 2: Run search full verification**
...
- [ ] **Step 3: Run rename full verification**
...
- `README.md`: `dist/search-1.4.0.tpx`;
...
- `README.md`: `dist/rename-1.3.0.tpx`;
...
```bash
cd /Users/young/Documents/telepiplex/features/search
/Users/young/.cache/codex-runtimes/codex-primary-runtime/dependencies/python/bin/python3 /Users/young/Documents/telepiplex/tools/build_feature.py . dist/search-1.4.0.tpx
unzip -t dist/search-1.4.0.tpx

cd /Users/young/Documents/telepiplex/features/rename
/Users/young/.cache/codex-runtimes/codex-primary-runtime/dependencies/python/bin/python3 /Users/young/Documents/telepiplex/tools/build_feature.py . dist/rename-1.3.0.tpx
unzip -t dist/rename-1.3.0.tpx
```
...
git commit -m "chore(search): prepare 1.4.0"
```
...
git commit -m "chore(rename): prepare 1.3.0"
```
...
**Interfaces:**
- Produces: synchronized `feature/search` and `feature/rename` branches.
- Produces immutable `search-v1.4.0` and `rename-v1.3.0` tags.
- Produces successful GitHub Releases, package artifacts, and catalog updates.

- [ ] **Step 1: Verify exact branch divergence**

```bash
git -C /Users/young/Documents/telepiplex/features/search fetch origin --prune
git -C /Users/young/Documents/telepiplex/features/search rev-list --left-right --count feature/search...origin/feature/search

git -C /Users/young/Documents/telepiplex/features/rename fetch origin --prune
git -C /Users/young/Documents/telepiplex/features/rename rev-list --left-right --count feature/rename...origin/feature/rename
```
...
```bash
git -C /Users/young/Documents/telepiplex/features/search push origin feature/search
git -C /Users/young/Documents/telepiplex/features/rename push origin feature/rename
```
...
git -C /Users/young/Documents/telepiplex tag -a rename-v1.3.0 -m "Release rename 1.3.0"
git -C /Users/young/Documents/telepiplex push origin search-v1.4.0 rename-v1.3.0
```
...
- search 1.4.0 release succeeds;
- rename 1.3.0 release succeeds;
