...
class DownloadFeature:
    def __init__(self, *, config: dict, host, client, jobs=None, config_store=None):
        self.config = config
        self.host = host
        self.client = client
        self.runtime = None
    ...
    async def command(self, request: dict) -> dict:
        ...
        if command in {"magnet", "m"}:
            ...
            if not directories:
                return self._message("⚠️ download 配置中没有 save_directories。")
            ...
            keyboard = [[{
                "text": f"📁 {item['name']}",
                "callback_data": f"download:path:{index}",
        ...
        raise FeatureError("not_found", f"unknown download command: {command}")
    ...
    async def message(self, request: dict) -> dict:
        ...
        if stage == "refresh_token":
            ...
            try:
                ...
                logger.error("download_direct_auth_write_failed")
        ...
        return {
            "actions": [{
                ...
                "data": {"keyboard": [
                    [{
                        "text": "授权配置",
                        "callback_data": "download:config:auth",
                    }],
                    [{
                        "text": "保存目录",
            ...
            return {
                "actions": [{
                    ...
                    "data": {"keyboard": [
                        [{
                            "text": "编辑名称",
                            "callback_data": "download:config:edit:name",
                        }],
                        [{
                            "text": "编辑路径",
                            "callback_data": "download:config:edit:path",
                        }],
                        [{
                            "text": "删除目录",
                            "callback_data": "download:config:delete",
                        }],
                        [{
                            "text": "返回目录列表",
                            "callback_data": "download:config:back",
                        }],
                        self._exit_row(),
                    ]},
                }],
            ...
            return {
                "actions": [{
                    ...
                    "data": {"keyboard": [
                        [{
                            "text": "确认删除",
                            "callback_data": "download:config:delete:confirm",
                        }],
                        [{
                            "text": "返回目录列表",
                            "callback_data": "download:config:back",
    ...
    def _bounded_index(items, value):
        ...
        if index < 0 or index >= len(items):
            raise IndexError("download directory index is out of range")
        return index
        ...
        return {
            "actions": [{
                ...
                "data": {"keyboard": [[
                    ...
                    {
                        "text": "115 扫码授权",
                        "callback_data": "download:auth:scan",
                    },
                ], self._exit_row()]},
            }],
            "session": {"state": "open"},
    ...
    def _directory_list_message(
        ...
        *,
        ...
        keyboard = [[{
            "text": f"📁 {item['name']}",
            "callback_data": f"download:config:item:{index}",
        ...
        if page_count > 1:
            ...
            if page > 0:
                navigation.append({
                    "text": "上一页",
                    "callback_data": f"download:config:page:{page - 1}",
                })
        ...
        keyboard.extend([
            [{"text": "新增目录", "callback_data": "download:config:add"}],
            [{
                "text": "保存并完成",
                "callback_data": "download:config:save",
            }],
            self._exit_row(),
        ])
        if operation is None:
    ...
    def _expire_sensitive_session(self, key, expected):
        if self.sessions.get(key) is expected:
            ...
            if operation_id in self.operations:
                ...
                try:
                    asyncio.create_task(self.host.report_operation(operation))
                except RuntimeError:
                    pass
    ...
    async def _start_scan_auth(self, request):
        ...
        try:
            ...
            logger.error(
                "download_scan_auth_start_failed "
                f"error={type(exc).__name__}"
            )
            self._clear_auth_session(key)
            operation = self._advance_operation(
            ...
            return result
        logger.info(
            "download_scan_auth_started "
            f"user_id={int(request.get('user_id') or 0)} "
            f"auth_uid={authorization['uid']}"
        )
        task_id = f"download-auth-{authorization['uid']}"
        self._clear_auth_session(key)
    ...
    async def _complete_scan_auth(self, authorization, operation_id):
        ...
        try:
            ...
            if isinstance(exc, TokenPersistenceCancelled):
                logger.info(
                    "download_scan_auth_token_rollback "
            ...
            if persisted is not None:
                ...
                if (
                    ...
                    raise FeatureError(
                        ...
                        "persisted download download belongs to another operation",
                    )
        ...
        if job_id in self.active_job_ids:
            ...
            if operation is None:
                raise FeatureError(
                    ...
                    "active download download has no operation owner",
                )
            ...
            if operation["operation_id"] != operation_id:
                raise FeatureError(
                    "idempotency_conflict",
                    "active download download belongs to another operation",
                )
            logger.info(
                "download_download_duplicate "
        ...
        try:
            ...
            if not _ambiguous_host_report_error(exc):
                ...
                self._advance_operation(
                    ...
                    state="failed",
                    stage="preparing_submission",
                    status_text="Host 未接受 115 下载任务所有权。",
                    control="",
                )
                raise
            report_error = exc
    ...
    async def _confirm_download_ownership(self, operation):
        ...
        if not isinstance(response, dict) or response.get("accepted") is not True:
            raise FeatureError(
                "ownership_rejected",
                "Host did not confirm download download ownership",
            )
    ...
    async def operation_control(self, request: dict) -> dict:
        ...
        operation = self.operations.get(operation_id)
        if operation is None:
            raise FeatureError("not_found", "download operation was not found")
        ...
        if action != "cancel":
            raise FeatureError("invalid_control", "download operation control is invalid")
            ...
            if self.jobs and operation.get("job_id"):
                ...
                if job is not None:
                    self.jobs.update(
                        ...
                        "cancelled",
                        result=job.get("result") or {},
                        error="cancelled before rename accepted",
                    )
            return {"actions": [], "operation": terminal}
    ...
    async def _report_operation(self, operation_id, **changes):
        ...
        if operation["chat_id"] and operation["user_id"]:
            ...
            if not isinstance(response, dict) or response.get("accepted") is not True:
                ...
                raise FeatureError(
                    "operation_rejected",
                    "Host rejected download operation ownership",
                )
        return operation
    ...
    @staticmethod
    def _exit_row():
        return [{"text": "退出", "callback_data": "download:exit"}]
