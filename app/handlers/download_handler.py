...
import init
from app.core.module_registry import DownloadCompletedEvent, DownloadRequest, PostDownloadResult
...
def _request_values(link_or_request, selected_path, user_id, target_folder_name, naming_metadata, metadata):
    if isinstance(link_or_request, DownloadRequest):
        request = link_or_request
        return (
            request.link,
    ...
    return link_or_request, selected_path, user_id, target_folder_name, naming_metadata, metadata


def _run_post_download_pipeline(event: DownloadCompletedEvent) -> PostDownloadResult:
    registry = getattr(init, "module_registry", None)
    if registry is None:
        return PostDownloadResult(False, final_path=event.final_path)
    return registry.run_post_download_pipeline(event)


def download_task(link, selected_path=None, user_id=None, target_folder_name=None, naming_metadata=None, metadata=None):
    info_hash = ""
    link, selected_path, user_id, target_folder_name, naming_metadata, metadata = _request_values(
        link,
        selected_path,
        user_id,
        target_folder_name,
    ...
    try:
        ...
        event = DownloadCompletedEvent(
            ...
            metadata=metadata,
            provider="115",
            storage=init.openapi_115,
        )
        pipeline_result = _run_post_download_pipeline(event)
        if pipeline_result.handled:
            if pipeline_result.message:
                add_task_to_queue(user_id, None, message=pipeline_result.message)
            return

        final_path = pipeline_result.final_path or event.final_path
        message = f"✅ 离线下载完成，已保存。\n\n保存目录：`{final_path}`"
        add_task_to_queue(user_id, None, message=message)
    except Exception as e:
