...
from app.runtime.command_catalog import (
    build_bot_commands,
    build_start_help,
    sync_bot_commands,
)
...
HOST_BOT_COMMANDS = build_bot_commands(None)
...
def build_plugin_manager(config=None, host_database=None):
    config = config or {}
    plugin_config = config.get("plugins") or {}
    root = Path(str(plugin_config.get("root") or "/config/plugins"))
    if host_database is None:
        host_database = root.parent / "host.db"
...
def queue_host_startup_notice():
    ...
    return add_task_to_queue(
        allowed_user,
        None,
        message=build_host_startup_notice_text(),
    )
...
async def send_plugin_update_notification(application, update):
    ...
    confirm_data = f"host-plugin-update:confirm:{update.reference}"
    decline_data = f"host-plugin-update:decline:{update.reference}"
...
async def reload(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if init.logger:
            init.logger.error(f"Host 配置读取失败: {type(exc).__name__}")
        await send_bot_message_safely(
            context.bot,
            chat_id=update.effective_chat.id,
            text="❌ Host 配置读取失败；继续使用重载前配置，Feature 未重载。",
        )
        return
    ...
    if manager is None:
        await send_bot_message_safely(
            context.bot,
            chat_id=update.effective_chat.id,
            text="❌ Host 配置已读取，但 Feature 管理器不可用；请重启 Host。",
        )
        return
    ...
    try:
        ...
        if init.logger:
            init.logger.error(f"Host 配置值无效: {type(exc).__name__}")
        await send_bot_message_safely(
            context.bot,
            chat_id=update.effective_chat.id,
            text="❌ Host 配置值无效；继续使用重载前配置，Feature 未重载。",
        )
        return
    ...
    lines = [
        ...
        "",
        "Host 已应用：" + ("、".join(hot_fields) if hot_fields else "无变更"),
        "",
        "Feature 重载结果：",
    ]
...
async def set_bot_menu(application):
    ...
    if await sync_bot_commands(application, router):
        ...
        return True
    if init.logger:
        init.logger.error("设置Bot菜单失败，后续 Feature 生命周期或 Host 重启时将重试。")
    return False
...
def configure_application(application, manager):
    ...
    application.add_handler(CallbackQueryHandler(
        plugin_install_callback,
        pattern=r"^host-plugin-install:",
    ))
    application.add_handler(CallbackQueryHandler(
        plugin_update_callback,
        pattern=r"^host-plugin-update:",
    ))
...
if __name__ == "__main__":
    ...
    try:
        asyncio.run(run_application_polling(
            application,
            after_start=lambda: start_host_runtime(application, plugin_manager),
        ))
