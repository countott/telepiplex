...
def log_runtime_features():
    ...
    init.logger.info(
        "Telepiplex runtime features: media_search=enabled, "
        "direct_metadata_link_search=enabled, builtin_douban_title_priority=latin_or_original_first, "
        "prowlarr_indexer_summary=enabled, search_command=enabled, search_short_command=enabled, "
        "download_contract=handoff_only, revision=%s" % revision
    )
...
def get_help_info():
    version = get_version()
    return f"""
<b>Telepiplex Media Search {version}</b>\n\n
<b>命令列表</b>\n
<code>/start</code> - 显示媒体搜索能力说明\n
<code>/reload</code> - 重载配置\n
<code>/search</code> - 搜索片源并选择候选\n
<code>/s</code> - /search 的短命令\n\n
此分支只包含媒体搜索能力。候选确认后会调用 <code>download_task</code> 合同，实际 115 投递由 main 缝合。
"""
...
def get_bot_menu():
    return [
        BotCommand("start", "获取媒体搜索说明"),
        BotCommand("reload", "重载配置"),
        BotCommand("search", "搜索片源"),
        BotCommand("s", "搜索片源"),
    ]
