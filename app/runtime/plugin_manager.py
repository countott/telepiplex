...
from app.runtime.plugin_catalog import CatalogError, ResolvedArtifact
from app.runtime.plugin_contract import HOST_API_VERSION
from app.runtime.plugin_store import ActiveRelease, PluginStore, StagedRelease, StoreError
from app.runtime.plugin_supervisor import (
    PluginProcess,
    PluginSupervisor,
    RoutedPluginClient,
    SupervisorError,
...
class PluginManager:
    def __init__(
        ...
        *,
        ...
        self.broker = broker
        self.host_api_version = str(host_api_version)
    ...
    async def available_updates(self):
        ...
        if not installed:
            return []
        return await discover(installed, self.host_api_version)
    ...
    def _verify(self, artifact_path: Path, expected_sha256: str):
        try:
            ...
            raise PluginOperationError(exc.code, self._sanitize(str(exc))) from None
        if not verified.manifest.supports_host(self.host_api_version):
            raise PluginOperationError(
                "incompatible_host",
                f"Feature requires host API {verified.manifest.host_api}; current is {self.host_api_version}",
            )
        return verified
