...
class PluginManifest:
    ...
    def from_mapping(cls, value: dict) -> "PluginManifest":
        ...
        host_api = _text(value.get("host_api"), "host_api")
        if not _HOST_RANGE.fullmatch(host_api):
            _invalid(f"host_api has invalid range: {host_api}")
    ...
    def supports_host(self, version: str) -> bool:
        version_match = _HOST_VERSION.fullmatch(str(version or "").strip())
        range_match = _HOST_RANGE.fullmatch(self.host_api)
