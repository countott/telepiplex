...
class FeatureBuilderTest(unittest.TestCase):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with (
                ...
                mock.patch.object(
                    build_feature,
                    "subprocess",
                    side_effect=AssertionError("Git metadata must not be read"),
                ),
            ):
                build_feature.main(
                    [
                        str(ROOT / "examples/echo_feature"),
                        str(output),
                        "--repository",
                        "countott/telepiplex",
                        "--branch",
            ...
            build.assert_called_once_with(
                (ROOT / "examples/echo_feature").resolve(),
                output,
                sdk_source=ROOT / "sdk",
                repository="countott/telepiplex",
                branch="main",
                commit="a" * 40,
            )
    ...
    def test_coordinated_feature_1_1_artifact_set_when_supplied(self):
        ...
        for artifact in verified:
            with self.subTest(plugin_id=artifact.manifest.plugin_id):
                ...
                self.assertTrue(artifact.manifest.supports_host("1.1"))
    ...
    def test_rejects_feature_source_importing_host_or_telegram(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            for name, content in (
                ("host.py", "from app.runtime import plugin_manager\n"),
                ("init.py", "import init\n"),
    ...
    def test_rejects_unsafe_requirement_sources(self):
        ...
        for requirement in (
            "-r sibling.txt\n",
            "https://example.test/telepiplex_download.whl\n",
            "./telepiplex_download.whl\n",
        ):
            with self.subTest(requirement=requirement), self.assertRaises(
                FeatureBuildError
            ):
    ...
    def test_rejects_sibling_feature_distribution_in_plugin_wheel_metadata(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            _write_wheel(
                ...
                "Name: example\n"
                "Version: 1.0.0\n"
                "Requires-Dist: telepiplex-download\n",
            )
    ...
    def test_rejects_invalid_requires_dist_in_plugin_wheel_metadata(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            _write_wheel(
                ...
                "Name: example\n"
                "Version: 1.0.0\n"
                "Requires-Dist: requests, telepiplex-download\n",
            )

            with self.assertRaises(FeatureBuildError):
                validate_plugin_wheel(wheel)

    def test_rejects_invalid_plugin_wheel_host_metadata_fields(self):
        from tools.build_feature import FeatureBuildError, validate_plugin_wheel
