...
from tools import update_feature_catalog
...
class FeatureCatalogUpdaterTest(unittest.TestCase):
    ...
    def _artifact(
        ...
        manifest = {
            ...
            "version": version,
            "host_api": ">=1.0,<2.0",
            "entry_point": "telepiplex_search.runtime:main",
    ...
    def _previous_catalog(self):
        return {
            "schema_version": 1,
            "release": "download-v1.0.1",
            "plugins": {
                "download": {
                    "versions": {
                        "1.0.1": {
                            "url": "https://example.test/download-1.0.1.tpx",
                            "sha256": "1" * 64,
                        }
                    }
                },
                "search": {
                    "versions": {
                        "1.2.2": {
                            "url": "https://example.test/search-1.2.2.tpx",
                            "sha256": "2" * 64,
                        }
                    }
                },
    ...
    def test_parses_supported_feature_tag(self):
        self.assertEqual(
            parse_feature_tag("search-v1.2.3"),
            ("search", "1.2.3"),
        )

    def test_maps_supported_features_to_monorepo_source_directories(self):
        self.assertEqual(
            update_feature_catalog.FEATURE_SOURCE_DIRS,
            {
    ...
    def test_accepts_main_as_feature_source_identity(self):
        artifact = self._artifact(branch="main")
        ...
        self.assertEqual(
            merged["plugins"]["search"]["versions"]["1.2.3"]["source"],
    ...
    def test_merge_preserves_other_plugins_and_versions(self):
        ...
        self.assertEqual(merged["plugins"]["download"], previous["plugins"]["download"])
        self.assertIn("1.2.2", merged["plugins"]["search"]["versions"])
        self.assertEqual(previous, original)
        self.assertEqual(merged["release"], "search-v1.2.3")
        entry = merged["plugins"]["search"]["versions"]["1.2.3"]
        self.assertEqual(
            entry["url"],
            "https://github.com/countott/telepiplex/releases/download/"
            "search-v1.2.3/search-1.2.3.tpx",
        )
    ...
    def test_merge_drops_legacy_plugin_ids(self):
        previous = self._previous_catalog()
        previous["plugins"].update({
            "open115": {"versions": {"1.2.3": {}}},
            "media-search": {"versions": {"1.5.0": {}}},
            "renaming": {"versions": {"1.3.0": {}}},
            "plex-management": {"versions": {"1.2.0": {}}},
        })
    ...
    def test_rejects_tag_manifest_and_branch_identity_mismatches(self):
        cases = (
            ("plugin", self._artifact(plugin_id="download")),
            ("version", self._artifact(version="1.2.4")),
            ("branch", self._artifact(branch="feature/rename")),
        )
    ...
    def test_rejects_reused_version_when_immutable_identity_changes(self):
        ...
        cases = {
            "digest": ("sha256", "0" * 64),
            "branch": ("source", {"branch": "feature/rename", "commit": "a" * 40}),
            "commit": (
                "source",
        ...
        for label, (field, value) in cases.items():
            with self.subTest(label=label):
                ...
                previous["plugins"]["search"]["versions"]["1.2.3"][field] = value
    ...
    def test_rejects_present_null_previous_version_entry(self):
        ...
        previous["plugins"]["search"]["versions"]["1.2.3"] = None
    ...
    def test_rejects_invalid_repository_and_artifact_filename(self):
        artifact = self._artifact()
        with self.assertRaises(CatalogUpdateError):
            merge_feature_release(None, artifact, "not a repo", "search-v1.2.3")
