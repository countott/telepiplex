...
TELEPIPLEX_WORKFLOW = ROOT / ".github/workflows/release.yml"
...
class ReleaseWorkflowTest(unittest.TestCase):
    ...
    def test_telepiplex_release_accepts_only_telepiplex_tags(self):
        workflow = self._workflow(TELEPIPLEX_WORKFLOW)
        triggers = workflow["on"]

        self.assertEqual(set(triggers), {"push"})
        self.assertEqual(triggers["push"]["tags"], ["telepiplex-v*"])
    ...
    def test_telepiplex_release_tests_pushes_image_and_publishes_latest_release(self):
        ...
        self.assertEqual(
            set(jobs),
            {"validate-telepiplex", "build-telepiplex-image", "publish-telepiplex-release"},
        )
        ...
        self.assertIn('--title "Telepiplex ${RELEASE_TAG#telepiplex-v}"', create)
    ...
    def test_telepiplex_manifest_probe_fails_closed(self):
        workflow = self._workflow(TELEPIPLEX_WORKFLOW)
        probe = self._step(
            workflow, "build-telepiplex-image", "Refuse an existing immutable image tag"
        )["run"]
        docker = """
        ...
        """
        base_env = {
            "RELEASE_TAG": "telepiplex-v1.2.3",
            "TELEPIPLEX_IMAGE": "ghcr.io/example/telepiplex",
        }

        for label, status, output, succeeds in (
            ("missing", "1", "manifest unknown", True),
            ("missing-code", "1", "MANIFEST_UNKNOWN", True),
            ("missing-ghcr", "1", "ERROR: ghcr.io/example/telepiplex:1.2.3: not found", True),
    ...
    def test_feature_release_builds_from_main_monorepo_source(self):
        workflow = self._workflow(FEATURE_WORKFLOW)
        steps = workflow["jobs"]["build-feature"]["steps"]
        names = [step.get("name") for step in steps]
        source = FEATURE_WORKFLOW.read_text(encoding="utf-8")

        self.assertNotIn("Checkout fixed Feature branch", names)
        resolve = self._step(
            workflow,
            "build-feature",
            "Resolve immutable Feature identity",
        )["run"]
        self.assertIn("FEATURE_SOURCE_DIRS", resolve)
        self.assertIn("source_dir=", resolve)

        build = self._step(
            workflow,
            "build-feature",
            "Build new Feature artifact",
        )["run"]
        self.assertIn('python tools/build_feature.py "$FEATURE_DIR"', build)
        self.assertIn('--repository "$GITHUB_REPOSITORY"', build)
        self.assertIn('--branch "$FEATURE_BRANCH"', build)
        self.assertIn('--commit "$GITHUB_SHA"', build)
        self.assertNotIn("feature-src", source)

    def test_feature_manifests_declare_main_source_branch(self):
        for plugin_id in (
            "download",
            "search",
            "rename",
            ...
            path = ROOT / "features" / plugin_id / "manifest.yaml"
            manifest = yaml.safe_load(path.read_text(encoding="utf-8"))
            with self.subTest(plugin_id=plugin_id):
                self.assertEqual(manifest["plugin_id"], plugin_id)
                self.assertEqual(manifest["source"]["branch"], "main")
                        ...
                        "assets": [
                            {
                                ...
                                "browser_download_url": (
                                    ...
                                    f"releases/download/search-v1.2.3/{asset}"
                                ),
                            }
                        ],
                    }
    ...
    def test_prior_catalog_asset_url_is_exact_and_never_receives_token(self):
        ...
        expected = (
            ...
            "platform-v1.0.5/search-1.2.3.tpx"
        )
        urls = {
            "valid": (expected, True),
            ...
            "wrong-path": (expected.replace("search-1.2.3.tpx", "other.tpx"), False),
        }
    ...
    def test_workflows_install_local_wheel_build_backends_only_in_read_job(self):
        ...
        for package in ("setuptools", "wheel"):
            self.assertIn(package, host_install)
