...
class CommandCatalogTest(unittest.TestCase):
    ...
    def test_legacy_filter_keeps_tasks_and_aliases_only(self):
        from app.runtime.command_catalog import build_bot_commands, build_start_help

        self.router.activate(
            "download",
            manifest("download", name="115", commands=(
                ("magnet", "投递磁力链接"),
                ("m", "投递磁力链接"),
                ("auth", "配置 115 授权"),
                ("q", "退出当前会话"),
        ...
        self.router.activate(
            "search",
            manifest("search", name="Media Search", commands=(
                ("search", "搜索片源"),
                ("s", "搜索片源"),
                ("search_config", "配置媒体搜索"),
            )),
            object(),
        )
        self.router.activate(
            "rename",
            manifest("rename", name="Media Renaming", commands=(
                ("rename_config", "配置媒体整理"),
            )),
            object(),
        )
        ...
        self.assertEqual(
            names,
            ["start", "reload", "plugin", "config", "magnet", "m", "search", "s"],
        )
        for command in ("search", "s", "magnet", "m"):
            self.assertIn(f"<code>/{command}</code>", help_text)
        for command in ("search_config", "auth", "q", "rename_config"):
