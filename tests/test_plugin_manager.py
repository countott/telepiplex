...
class PluginManagerTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_reserved_host_config_command_can_be_private_to_config_schema(self):
        schema, default = self._editable_config()
    ...
    async def test_install_resolves_name_and_uses_catalog_digest(self):
        from app.runtime.plugin_catalog import ResolvedArtifact
    ...
    async def test_artifact_verification_and_extraction_do_not_block_host_loop(self):
    ...
    async def test_incompatible_host_and_venv_failure_leave_no_active_record(self):
        from app.runtime.plugin_manager import PluginOperationError, PluginManager

        with self.assertRaises(PluginOperationError) as raised:
            await self.manager.install(self._artifact(host_api=">=2.0,<3.0"))
        self.assertEqual(raised.exception.code, "incompatible_host")
    ...
    async def test_disable_and_enable_switch_routes_without_host_restart(self):
        import os
    ...
    async def test_failed_config_restore_reports_activation_rollback_failure(self):
        ...
        from app.runtime.plugin_manager import PluginOperationError
        from app.runtime.plugin_store import StoreError
    ...
    async def test_remove_large_release_tree_does_not_block_host_loop(self):
        await self.manager.install(self._artifact(
            plugin_id="removable",
    ...
    async def test_available_updates_compares_only_active_versions(self):
        ...
        class Resolver:
            ...
            async def available_updates(self, installed, host_api_version):
                self.calls.append((installed, host_api_version))
                return [SimpleNamespace(
                    plugin_id="echo",
        ...
        self.assertEqual(resolver.calls, [({"echo": "1.0.0"}, "1.2")])
