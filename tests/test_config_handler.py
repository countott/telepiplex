...
class ConfigHandlerTest(unittest.TestCase):
    ...
    def test_config_menu_only_exposes_115_configuration(self):
        keyboard = build_config_keyboard()
        callback_data = [button.callback_data for row in keyboard.inline_keyboard for button in row]
        labels = [button.text for row in keyboard.inline_keyboard for button in row]

        self.assertEqual(callback_data, ["config_select:115", "config_cancel"])
        self.assertNotIn("配置 Prowlarr", labels)
        self.assertNotIn("配置 Plex", labels)
        self.assertNotIn("配置 TVDB", labels)
