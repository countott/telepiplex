...
class CapabilityRouterTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_routes_capability_with_context_and_idempotency(self):
        ...
        router.activate(
            "download",
            self._manifest("download", provides=(("download.provider", True),)),
            provider,
        )
        router.activate(
            "search",
            self._manifest("search", requires=("download.provider",), commands=("search",)),
            FakeClient(),
        )
        ...
        self.assertEqual(router.command_route("search").plugin_id, "search")
    ...
    async def test_missing_requirement_refuses_activation_without_mutating_routes(self):
        ...
        with self.assertRaises(RoutingError) as raised:
            router.activate(
                "search",
                self._manifest("search", requires=("download.provider",)),
                FakeClient(),
            )
    ...
    async def test_provider_loss_blocks_dependents_but_not_unrelated_plugins(self):
        ...
        router.activate(
            "download",
            self._manifest("download", provides=(("storage.provider", True),)),
            FakeClient(),
        )
        router.activate(
            "rename",
        ...
        router.deactivate("download")

        status = router.plugin_status("rename")
