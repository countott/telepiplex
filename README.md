...
配置位于 `/config/plugins/open115/config.yaml`。Core `/config` 选择 open115 后，可进入“授权配置”或“保存目录”：授权支持分步录入 Access/Refresh Token 与 115 扫码，保存目录支持逐条新增、编辑和删除，并在“保存并完成”后统一原子写入、立即生效。保存路径从 115 根文件夹开始填写，例如 `series/live action`；末尾 `/` 可省略，但不要以 / 开头，因为 Telegram 会将它识别为命令。直接发送 `/auth` 仍会进入授权方式选择。两种授权路线及自动刷新只原子写回该 Feature 私有配置，Token 不进入消息与日志。
