...
它同时提供无状态的 `media.search.resolve_metadata`，供 direct magnet 下载后的 renaming 实时复用同一套证据门禁。用户确认后的 `media_metadata v1` 与 `naming_metadata` 仍按原合同传给 `download.provider`，再由下载完成事件交给 renaming；搜索证据、候选、评分、范围和关系选择均即用即弃，不创建媒体实体数据库。
...
```bash
python /opt/telepiplex/tools/build_feature.py . dist/media-search-1.5.0.tpx
```
