...
配置保存和 Feature 路由切换由 Core 作为一个任务管理。取消时 Core 使用旧配置和旧路由自动恢复；无法完整验证时保留人工检查提示。115 下载取消不会删除已下载内容；只有取得精确 InfoHash 时才删除对应离线任务记录，且使用不删除源文件的模式。
...
该流水线先推送 `linux/amd64` Core 镜像 `ghcr.io/<owner>/telepiplex-core:1.1.0` 和 `latest`，随后创建 `core-v1.1.0` 同名 GitHub Release，并强制设为 **Latest**。Core Release 不附带 Feature、catalog 或其他资产，也不会改动 Feature。四种 Feature 各自使用独立 tag，当前发布示例为：
...
升级顺序固定为：先发布并重启 Core 1.1，再逐个发布/更新 1.1 Feature；最后才更新 catalog 指向这些新工件。不要先把 `core_api: ">=1.1,<2.0"` 的 Feature 提供给仍运行 Core 1.0 的环境。Core 1.1 启动后，旧 1.0 Feature 可继续运行，但只有升级到 1.1 的 Feature 才提供完整任务协调协议。
