...
Core 的 `/start` 帮助和 Telegram 显式命令列表只展示可以独立发起任务的
Feature 命令，同时保留命令路由与旧入口兼容性。

## 展示规则

- Core 命令 `/start`、`/reload`、`/plugin`、`/config` 保持外显。
- media-search 外显 `/search`、`/s`。
- open115 外显 `/magnet`、`/m`。
- 配置命令 `/media_search_config`、`/auth`、`/renaming_config` 不外显。
- 会话控制命令 `/q` 不外显，因为交互中已有显式退出按钮。
- 未启用或不可路由的 Feature 继续不外显。
- 一个 Feature 过滤后没有命令时，不显示空分组。
- plex-management 将来启用时外显 `/plex`，不外显 `/plex_config`。
...
- `true`：明确外显。
- `false`：明确隐藏。
- 省略：为兼容已经发布且没有该字段的 Feature 1.1.0，Core 隐藏命令名后缀为
...
- 运行 Core 完整测试、编译、工作流与 whitespace 检查。
- 只推送 `feature/telepiplex-core`，不修改或发布 Feature 分支。
- 创建 `core-v1.1.1` 标签，等待 GitHub Actions 完成 Core 测试、GHCR 镜像和 Release。
