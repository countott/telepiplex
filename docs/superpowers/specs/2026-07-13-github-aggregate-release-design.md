...
- linux/amd64 Telepiplex Telepiplex OCI 镜像。
- download、search、rename、sync 四个 Linux tpx。
- 引用 GitHub Release HTTPS 资产并固定 SHA-256 的 catalog.yaml。
- 包含上述资产的不可变 GitHub Release。

发布过程不得把 Feature 源码复制进 Telepiplex 镜像，不得从 main 拼装业务代码，也不得静默覆盖已经存在的 Release。
...
2. 01B 更新发现：后续设计负责 Telepiplex 读取远程 catalog、比较已安装版本、Telegram 通知和一次确认更新。

01A 完成后，远程 catalog 已具备 01B 所需的版本、Host API、来源 commit、URL 和 digest。
...
Telepiplex 来源是触发 workflow 的 tag commit。四个 Feature 分别从 feature/115、feature/search、feature/rename 和 feature/sync checkout。
...
### 5.1 validate-telepiplex

- checkout tag 对应 Telepiplex。
- 安装 Python 3.12 依赖。
- 运行 Telepiplex 完整测试和 tracked Python 编译检查。
...
1. checkout Telepiplex tag。
2. checkout 指定 Feature branch 到隔离目录。
3. 使用 Telepiplex 的 tools/build_feature.py 构建 Linux tpx。
4. 使用 Telepiplex artifact verifier 复验 manifest、成员和 digest。
5. 校验 manifest branch 与 matrix branch 一致。
6. 上传单个 tpx artifact 供 release job 汇总。

### 5.3 build-telepiplex-image

- 使用 GHCR：ghcr.io/<owner>/telepiplex。
- 只构建并推送 linux/amd64。
- 发布不可变 semver tag 和滚动 latest tag。
- OCI labels 记录 repository、release version、Telepiplex commit 和构建时间。
...
- Release notes 列出 Telepiplex image、Telepiplex commit、每个 Feature 的 version、branch、commit 和 digest。

## 6. Catalog 格式

Catalog 顶层包含 schema_version 和 release。plugins 下每个 Feature 以 manifest version 为版本键；每个版本记录 GitHub Release HTTPS URL、64 位小写 sha256、host_api，以及 source.branch 和 source.commit。
...
新增 tools/generate_release_catalog.py，接受 repository、tag、output 和一个或多个 tpx 路径。脚本通过 Telepiplex 的 verify_tpx 读取 typed manifest，不信任文件名内容。输出按 plugin ID 和 version 排序，并在检测到重复版本、非 semver、非法 repository/tag、非 tpx、损坏资产或缺少四个必需插件时失败。

## 9. 测试与验收

- catalog generator 对相同输入产生字节一致输出。
- URL、digest、Host API 和 source branch/commit 与 tpx manifest 一致。
- 重复插件版本、缺少插件、损坏 tpx 和非法 tag 均失败。
- workflow 静态测试确认：
  - tag/dispatch 才发布；
  - Telepiplex 只构建 linux/amd64；
  - registry 是 GHCR；
  - 四个 Feature branch 完整且没有 main 业务拼装；
  - PR 不存在 push 路径；
  - Release 依赖 Telepiplex 测试和全部 Feature 构建。
- Telepiplex 完整测试、compileall、YAML 解析和 git diff --check 通过。
- 本轮只提交 main，不推送、不创建真实 tag、不触发真实 Release。
