...
Core 已有 catalog 驱动的最新版选择和 Telegram callback，但旧部署的
...
Core 将官方 Release catalog 作为缺省来源。若配置恰好指向
...
安装按钮 callback 保持
`core-plugin-install:confirm:<plugin_id>@<target_version>`；更新按钮保持
`core-plugin-update:confirm:<plugin_id>@<target_version>`。两者都只允许授权用户触发，
...
- `/plugin` 对未安装 `open115` 显示安装按钮，callback 携带精确最新版引用。
- 已安装 `open115@1.0.0` 且 catalog 有 `1.1.0` 时显示更新按钮，callback 携带
  `open115@1.1.0`。
