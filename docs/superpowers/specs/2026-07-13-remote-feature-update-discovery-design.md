...
解析 release 时要求 plugin ID、semver、64 位小写 SHA-256、HTTPS tpx URL、host_api 和 source branch/commit 合法。现有 name@version resolve 继续复用同一缓存和 digest 校验。

## 3. 更新比较

PluginCatalog 暴露 available_updates(installed_versions, host_api_version)：

- 只比较当前 active Feature。
- 使用 PEP 440 Version 比较 semver。
- 只选择大于当前版本的最高稳定版本。
- catalog host_api 必须包含当前 Host API。
...
- catalog 版本比较覆盖最新版本、相同版本、旧版本、不兼容 Host API、无效 digest 和多个插件。
