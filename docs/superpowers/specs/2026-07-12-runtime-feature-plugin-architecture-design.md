...
`main` as the Docker/runtime source and one `feature/*`
...
The daily runtime image is built only from `main`. `main` is
...
- Feature lifecycle operations do not restart host.
- A host contract upgrade may restart the host container.
- 115 is a Feature, not part of host.
- Features depend on named capabilities and events, never another Feature name
  or Python package.
- Feature configuration and state are separated from host configuration.
...
The host process owns the Telegram connection, administrator authorization,
...
├── /config/plugins/download/.../venv        -> download process
├── /config/plugins/search/.../venv   -> search process
├── /config/plugins/rename/.../venv       -> rename process
└── /config/plugins/sync/.../venv -> plex process
```
...
- `main` contains host runtime, contract schemas, SDK,
  packaging CLI, Dockerfile, and host tests.
- `feature/115` contains only the 115 Feature source, manifest, config schema,
  locked dependencies, migrations, and tests.
- `feature/search`, `feature/rename`, and
  `feature/sync` follow the same Feature layout.
...
build timestamp, host API range, and SHA-256 digest. Local development installs
the same artifact into a running host through a local-path catalog entry; CI
and production install the published copy of that artifact.

No Feature branch carries a private copy of the host bot runtime. Contract
...
- compatible `host_api` version range;
...
`/config/config.yaml` contains only host settings: Telegram credentials,
...
The durable event journal is stored in `/config/host.db`. Delivery is
...
3. Verify archive paths, checksums, identity, version, and host API range.
...
fails, host restores old routes and the old process. `/plugin rollback`
...
Before a host image upgrade, `/plugin doctor` reports installed Features that
will become incompatible. A new host contract may require one explicit
...
4. Migrate search to the Telegram gateway and `download.provider` RPC.
5. Migrate rename to `download.completed`, `storage.provider`, and
   `media.organized`.
...
- A host-only Docker container starts with no business Feature code installed.
- Installing each `.tpx` artifact through host requires no host restart.
...
- No Feature imports host internals, Telegram, `init`, or another Feature.
- A Feature crash cannot terminate host or unrelated Features.
...
- Feature config/state and releases survive host container recreation through
  `/config`.
- Local development and production install the identical artifact built from a
  recorded source branch and commit SHA.
- End-to-end tests prove download -> search -> download completion ->
  rename -> media.organized -> Plex without a combined `main` runtime.

## 17. Deferred Scope

- Hostile-code sandboxing inside the shared container.
- Multiple host replicas or zero-downtime host contract upgrades.
