...
`plex-management` 不再重复管理 Plex 已经成熟的识别、匹配和元数据加载能力。

自动管线只负责：

1. `renaming` 发布 `media.organized` 后，按照 `plex_library_id` 触发 Plex 扫描；
...
- 有 Core `operation_id` 的协调任务保持现有所有权语义：重启后报告中断，不静默重放远端 Plex 写操作。
...
- Feature 版本升为 `1.2.0`。
- `core_api` 为 `>=1.2,<2.0`，因为 Telegram 图片选择使用 Core API 1.2 的 `send_photo` / `edit_photo` action。
