...
4. 用户确认界面只有文本。相同标题、年份接近或作品关系复杂时，没有海报这一强视觉证据；同时 Core 当前只支持安全的 `send_message` 和 `edit_message` 动作。
...
- media-search 负责作品身份与作品关系；renaming 只负责下载文件到既定集号/路径的映射。
...
- 不让 renaming 再次判断作品身份、电影/剧集关系或目标剧集。
...
- media-search 拥有请求图、证据适配、关系发现、评分、候选确认、实体注册表和 `media_metadata` 生成。
- telepiplex-core 只扩展安全图片动作和异步操作渲染，不理解媒体评分。
- renaming 消费已确认的 `media_metadata`，不重新搜索作品身份；确定性文件映射失败时，AI 仅可在锁定的集号集合内映射文件。
...
SQLite 文件位于 media-search 的 Feature 状态目录：`state/media_entities.db`。数据库只存已选中的规范实体，不存请求数据。
...
| `canonical_latin_title` | 传给 renaming 的拉丁标题 |
...
- 官方英文翻译只写入 `official_english_title` 作为参考，不参与默认 Prowlarr 查询和 renaming。
...
为兼容现有 SDK 和 renaming，`identity.english_title` 的运行时语义明确为“规范拉丁标题”：日文填罗马字，其他语言填官方英文。
...
Core 负责 URL/文本/键盘校验和 Telegram API 调用。图片发送或编辑失败时，执行一次安全降级：发送纯文本候选卡片，保留同一键盘和候选回调。不得因为海报失败丢失搜索任务。
...
- renaming 的 `media.search.resolve_metadata` RPC 使用现有 120 秒调用截止时间；media-search 自身必须在 90 秒内成功或返回结构化 `metadata_unresolved`，预留传输和清理时间。
...
media-search 输出最终 confirmed `media_metadata v1`：
...
- 没有元数据：通过 `media.search.resolve_metadata` 请求 media-search。
- media-search 无法确认：移入 `/未整理`，不启动 legacy TVDB + AI 身份兜底。
...
- 缺少 confirmed 元数据且 media-search 无法解析时进入 `/未整理`。
