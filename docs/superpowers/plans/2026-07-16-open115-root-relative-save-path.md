...
- Modify only `feature/open115`; do not change Core command routing.
...
- Modify: `src/telepiplex_open115/directories.py`
...
```bash
PYTHONPATH=src:/Users/young/Documents/telepiplex/.worktrees/telepiplex-core/sdk/src /Users/young/.cache/codex-runtimes/codex-primary-runtime/dependencies/python/bin/python3 -m unittest \
...
Add to `src/telepiplex_open115/directories.py`:
...
```bash
git add tests/test_feature_runtime.py src/telepiplex_open115/directories.py config.schema.json
git commit -m "fix(open115): use root-relative save paths"
```
...
```bash
PYTHONPATH=src:/Users/young/Documents/telepiplex/.worktrees/telepiplex-core/sdk/src /Users/young/.cache/codex-runtimes/codex-primary-runtime/dependencies/python/bin/python3 -m unittest \
  tests.test_feature_runtime.Open115FeatureTest.test_directory_working_copy_add_edit_delete_and_save \
  tests.test_feature_runtime.Open115FeatureTest.test_directory_input_rejects_invalid_and_duplicate_values \
  tests.test_feature_runtime.Open115FeatureTest.test_magnet_command_uses_session_and_namespaced_callback -v
```
...
git commit -m "fix(open115): accept Telegram-safe directory paths"
```
...
Build from the Core worktree using the verified source commit after the release commit:
