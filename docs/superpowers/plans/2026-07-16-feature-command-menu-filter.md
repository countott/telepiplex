...
- Publish the next unused Core patch tag `core-v1.1.1` only after fresh verification.
...
- Modify: `app/core/plugin_manifest.py`
...
**Files:**
- Modify: `tests/test_command_catalog.py`
- Modify: `app/core/command_catalog.py`
...
Activate legacy manifests exposing `/search`, `/s`, `/media_search_config`, `/magnet`, `/m`, `/auth`, `/q`, and `/renaming_config`. Assert both rendered surfaces contain only `/search`, `/s`, `/magnet`, and `/m`, and omit the empty renaming group. Add explicit override cases proving `menu_visible: true` can show a legacy-hidden name and `false` can hide an otherwise-visible task.
...
Run: `PYTHONPATH=.:sdk/src python -m pytest -q tests/test_command_catalog.py tests/test_plugin_manifest.py tests/test_telepiplex_core_surface.py`
...
```bash
git add app/core/plugin_manifest.py app/core/command_catalog.py tests/test_command_catalog.py
git commit -m "feat(core): filter advertised Feature commands"
```

### Task 3: Verify, push, and publish Core 1.1.1

**Files:**
- Verify only.

**Interfaces:**
- Consumes: committed Core command filtering.
- Produces: synchronized `origin/feature/telepiplex-core`, GHCR `1.1.1`/`latest`, and public `core-v1.1.1` Release.
...
- [ ] **Step 2: Push the Core branch and verify equality**

Run: `git push origin feature/telepiplex-core`, fetch, then require `origin/feature/telepiplex-core...HEAD` to report `0 0`.

- [ ] **Step 3: Create and push the release tag**

Create annotated tag `core-v1.1.1` at the verified Core commit and push it to `origin`.

- [ ] **Step 4: Monitor and verify publication**

Require every GitHub Actions job to conclude `success`, the public Latest Release to be `core-v1.1.1`, and GHCR `1.1.1` and `latest` to resolve to the same OCI digest containing `linux/amd64`.
