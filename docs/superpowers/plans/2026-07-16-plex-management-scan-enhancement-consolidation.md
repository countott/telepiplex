...
- Work only in `/Users/young/Documents/telepiplex/.worktrees/plex-management` on `feature/plex-management`.
...
- Feature version is `1.2.0`; state schema version is `2`.
...
- Modify: `src/telepiplex_plex/jobs.py`

**Interfaces:**
- Produces: `PlexManagementService.enqueue_organized_event(event: dict) -> dict | None`
- Produces: `PlexManagementService.run_job(job_id: int, *, should_cancel=None, on_stage=None) -> dict`
...
```bash
PYTHONPATH=src:../telepiplex-core/sdk/src \
...
```bash
git add src/telepiplex_plex/management.py src/telepiplex_plex/feature.py \
  src/telepiplex_plex/jobs.py tests/test_plex_management.py \
...
**Files:**
- Modify: `src/telepiplex_plex/rules.py`
- Modify: `src/telepiplex_plex/adapters/plex.py`
- Modify: `src/telepiplex_plex/management.py`
...
```bash
git add src/telepiplex_plex/rules.py src/telepiplex_plex/adapters/plex.py \
  src/telepiplex_plex/management.py tests/test_plex_rules.py \
...
- Produces: `PlexManagementService.scan_libraries(library_ids=None, *, should_cancel=None) -> dict`
...
```bash
git add manifest.yaml src/telepiplex_plex/runtime.py \
  src/telepiplex_plex/feature.py tests/test_feature_runtime.py
...
**Files:**
- Modify: `src/telepiplex_plex/mcp_server.py`
- Delete: `src/telepiplex_plex/ai.py`
...
```bash
git add src/telepiplex_plex/mcp_server.py tests/test_plex_mcp.py
git rm src/telepiplex_plex/ai.py tests/test_plex_ai.py
git commit -m "refactor(plex): reduce mcp to scan and enhancement tools"
```

---

### Task 5: Remove AI configuration and cut version 1.2.0
...
Assert `ai` is absent from defaults/schema/wizard, `/scan` exists in the manifest, version is `1.2.0`, and `state_schema_version` is `2`.
...
```text
dist/plex-management-1.2.0.tpx
```
...
```bash
git add src/telepiplex_plex/config_wizard.py config.default.yaml \
  ...
  tests/test_feature_runtime.py
git commit -m "chore(plex-management): cut 1.2.0"
```
