...
**Goal:** Close the three verified Core Feature release blockers while preserving the current Core API and all Feature business behavior.
...
- Modify only `feature/telepiplex-core`.
- Do not change Core API version, Feature manifests, or Feature business logic.
...
- Modify: `app/core/capability_router.py:63-81`
...
```bash
git add tests/test_plugin_manager.py app/core/capability_router.py
git commit -m "fix(core): preserve consumers during provider updates"
```
...
- Modify: `app/core/event_dispatcher.py:10-13`
...
```bash
git add tests/test_event_dispatcher.py app/core/event_dispatcher.py
git commit -m "fix(core): retry untyped Feature event failures"
```
...
validate_feature_requirements("https://example.test/telepiplex_open115.whl\n")
validate_feature_requirements("./telepiplex_open115.whl\n")
```

all raise `FeatureBuildError`. Add helper-generated wheels whose `METADATA` declares `Requires-Dist: telepiplex-open115` or `Name: telepiplex-open115`, and assert plugin metadata and final wheelhouse validation reject them. Retain a positive assertion for `requests>=2` and `telepiplex-plugin-sdk==1.0.0`.
...
**Interfaces:**
- Consumes: current Core branch and all four clean Feature worktrees.
...
Run each Feature worktree with `PYTHONPATH=src:../telepiplex-core/sdk/src` and `python3 -m unittest discover -s tests -t . -q`.

- [ ] **Step 3: Build and verify all four `.tpx` artifacts**

Use `tools/build_feature.py` against open115, media-search, renaming, and plex-management. Verify checksums, manifest source branch/commit, member allowlist, and absence of sibling Feature wheels.

- [ ] **Step 4: Run the real runtime matrix**

Install all four artifacts into one fresh Core manager using a short Unix runtime path. Assert all Features are healthy, capability and command routes exist, open115 removal is dependency-protected, a second manager restores all Features from persisted state, and Core PID remains unchanged.
