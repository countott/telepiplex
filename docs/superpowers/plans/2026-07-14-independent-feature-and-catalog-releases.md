...
**Goal:** Publish Telepiplex images, individual Feature artifacts, and the current Feature catalog through independent release channels.

**Architecture:** `telepiplex-v*` tags update only GHCR Telepiplex images. A Feature tag builds or reuses one immutable `.tpx` in a read-only job, creates one Feature Release in a write-scoped job, then uses an optimistic fetch/merge/non-fast-forward retry loop to update the `catalog` branch while preserving all unrelated catalog entries. Every Feature Release also carries the complete catalog snapshot so older `releases/latest` clients remain compatible.

**Tech Stack:** Python 3.12, PyYAML, Telepiplex `.tpx` verification, GitHub Actions, GitHub CLI on runners, Git/GHCR.

## Global Constraints

- Existing `platform-v1.0.5`, its GitHub Release, and its Telepiplex image remain immutable.
...
- Existing Telepiplex installations using `releases/latest/download/catalog.yaml` continue working.
...
def test_parses_supported_feature_tag():
    assert parse_feature_tag("search-v1.2.3") == ("search", "1.2.3")

def test_merge_preserves_other_plugins_and_versions():
    merged = merge_feature_release(previous, media_artifact, "countott/telepiplex", "search-v1.2.3")
    assert merged["plugins"]["download"] == previous["plugins"]["download"]
    assert "1.2.2" in merged["plugins"]["search"]["versions"]
```
...
git commit -m "feat(runtime): merge independent Feature releases into catalog"
```

### Task 2: Split Telepiplex and Feature workflows
...
**Interfaces:**
- Consumes: `telepiplex-v<semver>` and the four `<plugin-id>-v<semver>` tag families.
- Produces: a Telepiplex-only GHCR workflow and a split read-only-build/write-scoped one-Feature Release/catalog workflow with optimistic publication.
...
```python
assert telepiplex_triggers["push"]["tags"] == ["telepiplex-v*"]
assert "build-features" not in telepiplex_jobs
...
- [ ] **Step 3: Convert `release.yml` to Telepiplex-only release**

The workflow validates `telepiplex-v<semver>`, runs Telepiplex tests/compile, refuses an
...
git commit -m "ci: split Telepiplex and Feature releases"
```
...
Document Telepiplex tags, all Feature tag forms, the catalog branch, old-client
...
git commit -m "docs(runtime): document independent release channels"
```
...
**Interfaces:**
- Consumes: Tasks 1-3.
- Produces: a pushed `main` release-infrastructure commit.

- [ ] **Step 1: Run full Telepiplex tests and compilation**
...
- [ ] **Step 3: Push Telepiplex release infrastructure**
...
Expected: local and `origin/main` hashes match.
...
Expected versions: `download`, `search`, `rename`, and
`sync` are all `1.0.1`.

- [ ] **Step 2: Push tags sequentially from the Telepiplex infrastructure commit**

```bash
TELEPIPLEX_RELEASE_COMMIT=$(git rev-parse HEAD)
git tag download-v1.0.1 "$TELEPIPLEX_RELEASE_COMMIT"
git push origin download-v1.0.1
git tag search-v1.0.1 "$TELEPIPLEX_RELEASE_COMMIT"
git push origin search-v1.0.1
git tag rename-v1.0.1 "$TELEPIPLEX_RELEASE_COMMIT"
git push origin rename-v1.0.1
git tag sync-v1.0.1 "$TELEPIPLEX_RELEASE_COMMIT"
git push origin sync-v1.0.1
```
...
equal; Telepiplex `latest` remains the image produced by `platform-v1.0.5`.
