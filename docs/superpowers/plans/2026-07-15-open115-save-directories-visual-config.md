# open115 Save Directories Visual Config Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add a Telegram `/config` workflow that manages `save_directories` one entry at a time, saves the final list atomically, updates the running Feature immediately, and releases open115 1.2.0.

**Architecture:** Keep the existing open115 custom command and operation model. Add shared directory normalization in a focused module, extend `FeatureConfigStore` with a field-scoped atomic writer, and extend `Open115Feature` with a staged paginated directory editor whose working copy is committed only on explicit confirmation. Existing `/auth`, download, and Token persistence behavior remains independent.

**Tech Stack:** Python 3.9+, `asyncio`, PyYAML, `unittest`, Telepiplex Plugin SDK, Core `tools/build_feature.py`.

## Global Constraints

- Modify only `feature/open115` source and its local build output.
...
**Files:**
- Create: `src/telepiplex_open115/directories.py`
- Modify: `src/telepiplex_open115/config_store.py`
...
def _single_line(value, *, field: str) -> str:
    ...
    if not text or "\n" in text or "\r" in text:
        raise ValueError(f"open115 save directory {field} must be one non-empty line")
    return text
...
def normalize_save_directories(value) -> list[dict[str, str]]:
    ...
    for item in value:
        if not isinstance(item, dict) or set(item) != {"name", "path"}:
            raise ValueError("open115 save directory must contain only name and path")
        ...
        if name in names or path in paths:
            raise ValueError("open115 save directory name and path must be unique")
        names.add(name)
        paths.add(path)
...
```bash
git add src/telepiplex_open115/directories.py \
  src/telepiplex_open115/config_store.py tests/test_feature_runtime.py
git commit -m "feat(open115): persist visual save directories"
```
...
- Produces: `/config` home callbacks under `open115:config:*`.
- Produces: paginated directory callbacks `config:item:<index>` and `config:page:<page>` plus add/edit/delete/save actions.
- Preserves: `open115:auth:*`, `open115:path:*`, `open115:exit`, `/auth`, `/magnet`, `/m`, and `/q`.
...
Add this test helper to `Open115FeatureTest`:
...
```bash
python3 -m unittest \
  tests.test_feature_runtime.Open115FeatureTest.test_config_opens_home_while_auth_opens_authorization_directly \
  tests.test_feature_runtime.Open115FeatureTest.test_directory_list_is_paginated_with_bounded_keyboard -v
```
...
```python
[
    [{"text": "授权配置", "callback_data": "open115:config:auth"}],
    [{"text": "保存目录", "callback_data": "open115:config:directories"}],
    self._exit_row(),
]
```
...
```bash
python3 -m unittest tests.test_feature_runtime.Open115FeatureTest -v
```

Expected: the newly added directory management tests fail on unimplemented callbacks/stages while existing open115 behavior remains passing.
...
- [ ] **Step 8: Run the open115 feature tests and verify GREEN**
...
Expected: all `Open115FeatureTest` tests pass.

- [ ] **Step 9: Commit Task 2**

```bash
git add src/telepiplex_open115/service.py tests/test_feature_runtime.py
git commit -m "feat(open115): add visual save directory manager"
```
...
```bash
python /opt/telepiplex/tools/build_feature.py . dist/open115-1.2.0.tpx
```
...
git commit -m "chore(open115): prepare 1.2.0"
```

### Task 4: Full verification, push, and immutable build

**Files:**
- Verify: all tracked files in `feature/open115`.
- Create outside source worktree: `/Users/young/Documents/telepiplex/dist/open115-1.2.0.tpx`.

**Interfaces:**
- Consumes: clean committed `feature/open115` HEAD.
- Produces: pushed `origin/feature/open115` and an immutable `.tpx` whose manifest source commit is the pushed HEAD.
...
python3 -m py_compile src/telepiplex_open115/*.py tests/test_feature_runtime.py
git diff --check
```
...
git rev-parse HEAD
git ls-remote --heads origin feature/open115
```
...
Run from `/Users/young/Documents/telepiplex/.worktrees/telepiplex-core`:

```bash
python3 tools/build_feature.py \
  /Users/young/Documents/telepiplex/.worktrees/open115 \
  /Users/young/Documents/telepiplex/dist/open115-1.2.0.tpx
```
...
artifact = Path("/Users/young/Documents/telepiplex/dist/open115-1.2.0.tpx")
assert artifact.is_file() and artifact.stat().st_size > 0
with zipfile.ZipFile(artifact) as package:
    manifest = yaml.safe_load(package.read("manifest.yaml"))
assert manifest["plugin_id"] == "open115"
assert manifest["version"] == "1.2.0"
assert manifest["source"]["branch"] == "feature/open115"
print(artifact)
print(manifest["source"]["commit"])
PY
shasum -a 256 /Users/young/Documents/telepiplex/dist/open115-1.2.0.tpx
```

Expected: artifact metadata matches open115 1.2.0 and the embedded commit equals the pushed branch SHA.
