...
**Goal:** Make renaming consume media-search identity and relationships exclusively, while retaining AI only for file-to-locked-episode mapping and carrying Japanese romaji into paths.
...
- Work remains local to `feature/renaming` and is not pushed.
...
**Files:**
- Modify: `src/telepiplex_renaming/processor.py`
- Modify: `src/telepiplex_renaming/service.py`
- Modify: `src/telepiplex_renaming/ai.py`
- Test: `tests/test_feature_processor.py`

**Interfaces:**
- Consumes: confirmed `media_metadata`, or `media.search.resolve_metadata` response.
- Produces: deterministic `/未整理` result when media-search cannot return a confirmed contract; no calls to `search_tvdb_series` for identity fallback.
...
async def test_unresolved_media_search_moves_release_to_unorganized(self):
    core = FakeCore()
    core.media_search_result = {}
    feature = RenamingFeature(config={"unorganized_path": "/Unorganized"}, core=core)
    await feature._run_organization("job-1", self.payload_without_metadata(), "op-1")
    self.assertEqual(core.storage.moved, [("/Downloads/Release", "/Unorganized")])

@patch("telepiplex_renaming.processor.search_tvdb_series")
...
```bash
git add src/telepiplex_renaming/processor.py src/telepiplex_renaming/service.py src/telepiplex_renaming/ai.py tests/test_feature_processor.py
git commit -m "refactor(renaming): require media-search identity"
```
...
- Modify: `src/telepiplex_renaming/tvdb_rename.py` only if the current helpers do not pass the tests.
- Modify: `src/telepiplex_renaming/media_naming.py` only if the current helpers do not pass the tests.
...
```bash
git add src/telepiplex_renaming/tvdb_rename.py src/telepiplex_renaming/media_naming.py tests/test_tvdb_rename.py tests/test_feature_processor.py
git commit -m "test(renaming): lock Japanese romaji paths"
```
...
**Interfaces:**
- Produces: a renaming branch that requires confirmed identity and preserves constrained file mapping.
