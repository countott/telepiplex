...
Run: `python3 -m unittest tests.test_host_media_metadata -v`
...
```bash
git add sdk/src/telepiplex_plugin_sdk/media_metadata.py tests/test_host_media_metadata.py
git commit -m "feat(runtime): validate canonical media titles"
```
...
git commit -m "feat(runtime): render safe feature photo actions"
```
...
Run: `python3 -m unittest tests.test_host_media_metadata tests.test_plugin_handler tests.test_interaction_handler -v`
