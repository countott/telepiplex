...
**Goal:** Make the two-step Open115 directory editor explicitly accept a single root folder such as `真人电影` and publish Open115 1.2.3.
...
- Modify only `feature/open115`; do not change Core routing.
...
- Release the reviewed result as exactly `1.2.3`.
...
**Interfaces:**
- Consumes: `Open115Feature.message(request: dict) -> dict` during `directory_add_name` and `directory_add_path`.
...
```bash
PYTHONPATH=src:/Users/young/Documents/telepiplex/.worktrees/telepiplex-core/sdk/src /Users/young/.cache/codex-runtimes/codex-primary-runtime/dependencies/python/bin/python3 -m unittest tests.test_feature_runtime.Open115FeatureTest.test_directory_working_copy_add_edit_delete_and_save -v
```
...
```bash
PYTHONPATH=src:/Users/young/Documents/telepiplex/.worktrees/telepiplex-core/sdk/src /Users/young/.cache/codex-runtimes/codex-primary-runtime/dependencies/python/bin/python3 -m unittest tests.test_feature_runtime.Open115FeatureTest.test_directory_working_copy_add_edit_delete_and_save -v
PYTHONPATH=src:/Users/young/Documents/telepiplex/.worktrees/telepiplex-core/sdk/src /Users/young/.cache/codex-runtimes/codex-primary-runtime/dependencies/python/bin/python3 -m unittest discover -s tests -t . -v
```
...
```bash
git add tests/test_feature_runtime.py src/telepiplex_open115/service.py
git commit -m "fix(open115): clarify single-level save paths"
```

---

### Task 2: Prepare and publish Open115 1.2.3
...
- Modify: `README.md`
- Create outside the Feature worktree: `/Users/young/Documents/telepiplex/dist/open115-1.2.3.tpx`

**Interfaces:**
- Consumes: the verified single-level flow from Task 1.
- Produces: immutable Open115 `1.2.3` identity and `open115-v1.2.3` Release/catalog entry.

- [ ] **Step 1: Write failing version and README contract assertions**

Change `FeatureSourceContractTest` to expect manifest/project version `1.2.3`, README build path `dist/open115-1.2.3.tpx`, and README text containing both `单级目录` and `真人电影`.
...
Set `manifest.yaml` and `pyproject.toml` to `1.2.3`. Update README with the two-step `真人电影` example and change its build artifact path to `dist/open115-1.2.3.tpx`.

- [ ] **Step 4: Run release verification and commit**

```bash
PYTHONPATH=src:/Users/young/Documents/telepiplex/.worktrees/telepiplex-core/sdk/src /Users/young/.cache/codex-runtimes/codex-primary-runtime/dependencies/python/bin/python3 -m unittest discover -s tests -t . -v
/Users/young/.cache/codex-runtimes/codex-primary-runtime/dependencies/python/bin/python3 -m py_compile src/telepiplex_open115/*.py
git diff --check
git add tests/test_feature_runtime.py manifest.yaml pyproject.toml README.md
git commit -m "chore(open115): prepare 1.2.3"
```
...
Build with Core `tools/build_feature.py`, verify the local artifact identifies Open115 1.2.3 and the final `feature/open115` source commit, push `feature/open115`, then create and push annotated tag `open115-v1.2.3` on the current synchronized `feature/telepiplex-core` release-infrastructure commit.

Wait for `Publish one Telepiplex Feature` to succeed. Verify the public release is neither draft nor prerelease and contains exactly `open115-1.2.3.tpx`, `catalog.yaml`, and `catalog.yaml.sha256`. Download the Linux artifact, verify its checksum and manifest, and confirm `origin/catalog` contains Open115 1.2.3 with the same SHA-256, URL, branch and source commit.
