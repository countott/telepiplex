...
### Task 1: Host API 1.1 operation protocol
...
async def test_operation_report_uses_authenticated_feature_identity(self):
    ...
    client = HostClient(broker.socket_path, "echo-token")
...
```bash
python3 -m unittest tests.test_plugin_sdk_runtime tests.test_runtime_broker -v
```
...
Implement `HostClient.report_operation()` as `_request("operation.report", report, ...)`. Extend `FeatureRuntime._dispatch()` with optional `operation.control` and `operation.snapshot` business calls. Extend `RuntimeBroker._dispatch()` to pass reports only through the authenticated identity to `operation_sink`.

Update SDK version to `1.1.0`, runner context host range to `>=1.1,<2.0`, and Host API version to `1.1`. Keep every existing API 1.0 method unchanged.

- [ ] **Step 4: Verify GREEN**

```bash
python3 -m unittest tests.test_plugin_sdk_runtime tests.test_runtime_broker tests.test_plugin_runtime_e2e -v
```
...
```bash
git add sdk app/runtime/plugin_contract.py app/runtime/runtime_broker.py tests/test_plugin_sdk_runtime.py tests/test_runtime_broker.py
git commit -m "feat(runtime): add Feature operation protocol"
```
...
def test_late_revision_cannot_overwrite_cancelled_state(self):
    current = self.coordinator.report("search", self.report(revision=3, state="cancelled"))
    stale = self.coordinator.report("search", self.report(revision=2, state="running"))
...
def test_handoff_changes_owner_without_releasing_gate(self):
    self.coordinator.report("search", self.report(
        state="handed_off", next_plugin_id="download", revision=2,
    ))
...
Create an `operations` table in `host.db`. Validate these exact sets:
...
```bash
git add app/runtime/interaction_coordinator.py tests/test_interaction_coordinator.py
git commit -m "feat(runtime): persist coordinated interactions"
```
...
- Produces: callback namespace `host-operation:<action>:<operation_id>`.
...
Register a high-priority handler before command/callback/text routing. In `awaiting_input`, allow only plain text and callback namespaces owned by the active Feature. In running states, stop every ordinary message and command; allow only the matching `host-operation` control.
...
Build one coordinator from `host.db`, pass its authenticated sink to `RuntimeBroker`, attach a Telegram render listener after application creation, and query active Features through `operation.snapshot` after `manager.start()`. Mark rows with no confirmed owner `interrupted`.

- [ ] **Step 5: Verify GREEN**

```bash
python3 -m unittest tests.test_interaction_coordinator tests.test_interaction_handler tests.test_plugin_handler tests.test_bot_runtime_startup tests.test_runtime_broker -v
```
...
git commit -m "feat(runtime): coordinate Telegram Feature tasks"
```
...
**Files:**
- Create: `app/runtime/command_catalog.py`
- Modify: `app/115bot.py`
- Modify: `app/handlers/plugin_handler.py`
...
- Produces: `build_start_help(router, host_version: str) -> str`.
...
def test_combines_host_and_active_feature_commands(self):
    commands = build_bot_commands(router_with("download", "search"))
...
```bash
git add app/runtime/command_catalog.py app/115bot.py app/handlers/plugin_handler.py tests/test_command_catalog.py tests/test_bot_runtime_startup.py tests/test_plugin_handler.py
git commit -m "feat(runtime): publish active Feature commands"
```

---

### Task 5: download exits, task status, and cancellation

**Files (worktree `.worktrees/download`):**
- Modify: `manifest.yaml`
- Modify: `pyproject.toml`
- Modify: `src/telepiplex_download/client.py`
- Modify: `src/telepiplex_download/service.py`
- Modify: `src/telepiplex_download/runtime.py`
- Modify: `src/telepiplex_download/jobs.py`
- Test: `tests/test_feature_runtime.py`

**Interfaces:**
- Consumes: `HostClient.report_operation()` and `FeatureRuntime.operation_control`.
- Propagates: `operation_id` through download capability payload/result and download events.
- Produces: `DownloadFeature.operation_control(request: dict) -> dict`.
...
Assert every open auth/path response has exactly one exit, invalid token input retains it, QR polling is cancellable, and download progress reports `preparing`, `submitted`, `downloading`, `reading_files`, and `handoff_rename`. Test one offline-task deletion attempt when InfoHash is known and preservation when it is absent or deletion fails.
...
git commit -m "feat(download): expose cancellable task interactions"
```

---

### Task 6: search asynchronous stages and complete exits

**Files (worktree `.worktrees/search`):**
- Modify: `manifest.yaml`
- Modify: `pyproject.toml`
- Modify: `src/telepiplex_search/config_wizard.py`
...
**Interfaces:**
- Produces: `SearchFeature.operation_control()` and `operation_snapshot()`.
...
```python
async def test_submission_hands_operation_to_download(self):
    await feature._submit_release("plan-1", stored, "0", operation_id="op-1")
    self.assertEqual(host.calls[0][2]["operation_id"], "op-1")
    self.assertEqual(host.reports[-1]["state"], "handed_off")
    self.assertEqual(host.reports[-1]["next_plugin_id"], "download")
```
...
git commit -m "feat(search): coordinate cancellable search tasks"
```

---

### Task 7: rename stage reports and verified compensation

**Files (worktree `.worktrees/rename`):**
- Modify: `manifest.yaml`
- Modify: `pyproject.toml`
- Modify: `src/telepiplex_rename/config_wizard.py`
- Create: `src/telepiplex_rename/operations.py`
...
git commit -m "feat(rename): add cancellable staged organization"
```

---

### Task 8: sync task stages and cancellation

**Files (worktree `.worktrees/sync`):**
- Modify: `manifest.yaml`
- Modify: `pyproject.toml`
...
**Interfaces:**
- Produces: `SyncFeature.operation_control()` and `operation_snapshot()`.
- Consumes: `operation_id` from `media.organized`.
- Extends: `LibrarySyncService.run_job(job_id, *, should_cancel=None, on_stage=None)`.
...
**Interfaces:**
- Consumes: four Feature 1.1.0 artifacts and Host API 1.1.
...
self.assertEqual(coordinator.active(chat_id, user_id).plugin_id, "sync")
...
```bash
python3 -m build sdk
python3 tools/build_feature.py ../download ../download/dist/download-1.1.0.tpx
python3 tools/build_feature.py ../search ../search/dist/search-1.1.0.tpx
python3 tools/build_feature.py ../rename ../rename/dist/rename-1.1.0.tpx
python3 tools/build_feature.py ../sync ../sync/dist/sync-1.1.0.tpx
```
...
git commit -m "test(runtime): verify coordinated Feature pipeline"
```
