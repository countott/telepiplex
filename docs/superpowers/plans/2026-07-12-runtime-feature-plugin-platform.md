...
- Work only in `main` for this plan.
- Do not modify or merge `main`.
- Do not copy business Feature code into the host image.
- Host API starts at `1.0`; incompatible manifests fail closed.
- Feature packages are immutable `.tpx` ZIP artifacts with member checksums.
- Feature processes never import Telegram, `app.init`, host internals, or another Feature.
- Feature lifecycle actions must not restart the host process.
...
**Files:**
- Create: `app/runtime/plugin_contract.py`
- Create: `app/runtime/plugin_manifest.py`
- Test: `tests/test_plugin_manifest.py`

**Interfaces:**
- Produces: `HOST_API_VERSION = "1.0"`.
...
- Produces: `PluginManifest.supports_host(version: str) -> bool`.

- [x] **Step 1: Write manifest RED tests**

Test a valid manifest, missing identity, invalid semantic version, unsupported
host range, duplicate capabilities, duplicate commands, unsafe entry points,
...
assert manifest.supports_host("1.0")
```
...
Expected: import failure for `app.runtime.plugin_manifest`.
...
```bash
git add app/runtime/plugin_contract.py app/runtime/plugin_manifest.py tests/test_plugin_manifest.py
git commit -m "feat(runtime): define Feature plugin contract"
```

### Task 2: Deterministic artifact builder and verifier

**Files:**
- Create: `app/runtime/plugin_artifact.py`
...
Expected: import failure for `app.runtime.plugin_artifact`.
...
```bash
git add app/runtime/plugin_artifact.py tools/build_tpx.py tests/test_plugin_artifact.py
git commit -m "feat(runtime): build and verify immutable Feature artifacts"
```

### Task 3: Persistent plugin store and schema-owned configuration

**Files:**
- Create: `app/runtime/plugin_store.py`
- Test: `tests/test_plugin_store.py`
- Modify: `requirements.txt`
- Modify: `app/init.py`
- Modify: `app/config.yaml.example`
- Modify: `config/config.yaml.example`
- Delete: `config/modules/host.yaml.example`
- Test: `tests/test_category_route_startup.py`
- Test: `tests/test_config_template_contract.py`
- Test: `tests/test_telepiplex_surface.py`
...
Expected: import failure for `app.runtime.plugin_store`.

- [x] **Step 3: Add the schema dependency and host-only config**
...
```bash
git add app/runtime/plugin_store.py requirements.txt app/config.yaml.example config/config.yaml.example tests/test_plugin_store.py
git commit -m "feat(runtime): persist Feature releases and owned config"
```

### Task 4: Unix RPC protocol and Feature SDK runtime

**Files:**
- Create: `app/runtime/plugin_rpc.py`
...
- [x] **Step 4: Implement the host client**
...
```bash
git add app/runtime/plugin_rpc.py sdk tests/test_plugin_rpc.py tests/test_plugin_sdk_runtime.py
git commit -m "feat(runtime): add Unix RPC Feature SDK"
```

### Task 5: Capability router and durable event journal

**Files:**
- Create: `app/runtime/capability_router.py`
- Create: `app/runtime/event_journal.py`
...
```bash
git add app/runtime/capability_router.py app/runtime/event_journal.py tests/test_capability_router.py tests/test_event_journal.py
git commit -m "feat(runtime): route capabilities and journal Feature events"
```

### Task 6: Subprocess supervisor and health isolation

**Files:**
- Create: `app/runtime/plugin_supervisor.py`
...
Expected: import failure for `app.runtime.plugin_supervisor`.
...
```bash
git add app/runtime/plugin_supervisor.py tests/test_plugin_supervisor.py tests/fixtures/plugin_processes
git commit -m "feat(runtime): supervise isolated Feature processes"
```

### Task 7: Transactional plugin manager lifecycle

**Files:**
- Create: `app/runtime/plugin_manager.py`
...
Cover successful install, checksum failure, incompatible host API, pip failure,
config failure, handshake failure, missing capability, command conflict,
enable/disable without host restart, update shadow start, drain timeout with
...
Expected: import failure for `app.runtime.plugin_manager`.
...
Restore active releases independently so one corrupt plugin cannot stop host.
...
```bash
git add app/runtime/plugin_manager.py tests/test_plugin_manager.py
git commit -m "feat(runtime): manage transactional Feature lifecycle"
```
...
Register host commands first, then one generic command gateway and one generic
...
- [x] **Step 5: Wire host startup and shutdown**

Construct `PluginManager` from host config, store it in `application.bot_data`,
...
git commit -m "feat(runtime): control hot-pluggable Features from Telegram"
```
...
Start a host manager in a temporary root, record the current host PID, build and
...
Expected: all lifecycle states pass, the host PID is constant, and no child
...
**Interfaces:**
- Produces a host-only image with venv support, no business Feature source, and
  persistent `/config/plugins` storage.

- [x] **Step 1: Write deployment RED tests**

Assert the image contains host/SDK/installer only, includes the runtime packages
...
Run: `python3 -m unittest tests.test_deployment_contract tests.test_telepiplex_surface -v`

Expected: assertions fail against the old in-process module docs/config.

- [x] **Step 3: Update Docker and documentation**

Install host dependencies, copy `app`, `sdk`, and `tools`, create
`/config/plugins` with the runtime user permissions, expose no Feature ports,
and keep `CMD ["python", "115bot.py"]`. Document that Docker always pulls host
...
and schemas, scan `main` for business module entry points,
and confirm the other Feature worktrees remain unchanged.

- [x] **Step 5: Commit**

```bash
git add Dockerfile Dockerfile.local docker-compose.yaml README.md README_EN.md tests/test_deployment_contract.py tests/test_telepiplex_surface.py
git commit -m "feat(runtime): ship host-only hot Feature runtime"
```
...
- Create: `app/runtime/event_dispatcher.py`
- Create: `sdk/src/telepiplex_plugin_sdk/host_client.py`
...
2. `feature/search`: replace Telegram/host imports with command envelopes
   and `download.provider` RPC.
3. `feature/rename`: consume durable `download.completed`, call
   `storage.provider`, and publish `media.organized`.
4. `feature/sync`: consume `media.organized`, preserve interrupted
   job semantics, and expose `library.sync`.
5. Cross-branch artifact matrix: build all four `.tpx` files and run the entire
   business flow in a host-only container without `main`.
