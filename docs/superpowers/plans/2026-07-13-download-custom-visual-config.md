# download Custom Visual Config Implementation Plan
...
async def test_custom_config_feature_is_listed_and_handed_to_feature_session(self):
    ...
    manager.doctor = Mock(return_value=[
        {"plugin_id": "search", "state": "healthy"},
        {"plugin_id": "download", "state": "healthy"},
    ])
    client = AsyncMock()
    client.request.return_value = {
        "actions": [{
            ...
            "data": {"keyboard": [[{
                ...
                "callback_data": "download:auth:direct",
            }]]},
        }],
        "session": {"state": "open"},
    }
    ...
    route = SimpleNamespace(plugin_id="download", client=client, manifest=manifest)
    router = Mock()
    router.plugin_route.side_effect = lambda plugin_id: route if plugin_id == "download" else None
    ...
    self.assertIn("download", update.effective_message.reply_text.await_args.args[0])

    index = context.user_data["host_config_plugins"].index("download")
    update.callback_query.data = f"host-config-plugin:{index}"
    with patch("app.handlers.config_handler.init.check_user", return_value=True):
...
git commit -m "feat(runtime): delegate custom Feature configuration"
```

---

### Task 2: Restore the download two-step token wizard
...
**Interfaces:**
- Consumes: `FeatureConfigStore.write_tokens(access_token, refresh_token, auth_mode="direct")`, `client.set_tokens(...)`, Feature callback namespace `download`, and message routing keyed by `(chat_id, user_id)`.
...
- [ ] **Step 2: Run the download red tests**

Run:

```bash
python3 -m unittest tests.test_feature_runtime.DownloadFeatureTest tests.test_feature_runtime.FeatureSourceContractTest -v
```
...
Set `manifest.yaml` and `pyproject.toml` to version `1.0.1`, and update the README build output example to `dist/download-1.0.1.tpx`.
...
- [ ] **Step 5: Preserve scan behavior and run download tests**
...
python3 -m py_compile src/telepiplex_download/service.py src/telepiplex_download/runtime.py src/telepiplex_download/config_store.py
```
...
```bash
git add config.schema.json manifest.yaml pyproject.toml README.md src/telepiplex_download/service.py tests/test_feature_runtime.py
git commit -m "fix(download): restore visual token configuration"
```
...
git -c host.whitespace=blank-at-eol,blank-at-eof,space-before-tab,cr-at-eol diff --check origin/main...HEAD
...
- [ ] **Step 2: Run complete download verification**
...
python3 -m unittest discover -s tests -t .
python3 -m py_compile $(git ls-files '*.py')
git -c host.whitespace=blank-at-eol,blank-at-eof,space-before-tab,cr-at-eol diff --check origin/feature/115...HEAD
git status --short --branch
```
