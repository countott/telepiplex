...
- Feature work remains local to `feature/media-search` and is not pushed.
...
**Files:**
- Create: `src/telepiplex_media_search/entity_graph.py`
...
```bash
git add src/telepiplex_media_search/entity_graph.py src/telepiplex_media_search/adapters/douban.py src/telepiplex_media_search/adapters/tvdb.py tests/test_entity_graph.py tests/test_douban_adapter.py tests/test_tvdb_adapter.py
git commit -m "feat(media-search): build request entity graphs"
```

### Task 2: Resolve Canonical Search and Naming Titles

**Files:**
- Create: `src/telepiplex_media_search/title_policy.py`
...
```bash
git add src/telepiplex_media_search/title_policy.py tests/test_title_policy.py
git commit -m "feat(media-search): enforce canonical title policy"
```

### Task 3: Persist Only Selected Canonical Entities

**Files:**
- Create: `src/telepiplex_media_search/entity_registry.py`
...
```bash
git add src/telepiplex_media_search/entity_registry.py src/telepiplex_media_search/runtime.py tests/test_entity_registry.py
git commit -m "feat(media-search): persist selected canonical entities"
```

### Task 4: Implement Fixed Program and AI Scorecards

**Files:**
- Create: `src/telepiplex_media_search/candidate_score.py`
...
```bash
git add src/telepiplex_media_search/candidate_score.py src/telepiplex_media_search/ai.py tests/test_candidate_score.py tests/test_search_ai_pipeline.py
git commit -m "feat(media-search): add fixed candidate scorecards"
```
...
- Modify: `src/telepiplex_media_search/deterministic.py`
...
```bash
git add src/telepiplex_media_search/planner.py src/telepiplex_media_search/deterministic.py src/telepiplex_media_search/search_plan.py tests/test_ranked_planner.py tests/test_search_planner_service.py tests/test_deterministic_planner.py tests/test_search_plan.py
git commit -m "feat(media-search): build budgeted ranked plans"
```
...
git commit -m "feat(media-search): confirm poster-backed candidates"
```
...
**Interfaces:**
- Produces: locally tested `feature/media-search` with no unselected persistent data.
...
Run: `rg -n "CREATE TABLE" src/telepiplex_media_search`
