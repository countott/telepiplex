...
适用版本：Host 3.8.2（Host API 1.8）、Search 2.4.1、Rename 2.3.1、Sync 2.1.0。SDK 仍为 2.2.0。本次仅提升 Host、Search 和 Rename 的补丁版本。

## 2026-09-17 补丁修复

- 电影整理成功后直接结束；Rename 仅为剧集提供下一步上下文，Host 和 Search 同时拒绝电影接续入口。
- 作品确认移到后台，首次点击即占用确认状态；重复点击和同一 Telegram update 重放不会重复确认。失败后允许新的用户点击重试，取消后的迟到结果不能恢复旧任务。
- 确认身份优先使用最终所选海报。链接换图失败时上传本地图片，缺图或取图失败时上传所选片名占位图。旧候选图片请求迟到时仍以最新身份为准；身份消息未成功封存则停止后续片源搜索。

升级顺序：Host 3.8.2 → Search 2.4.1、Rename 2.3.1 → 发起新任务。下文保留 Host 3.8.1 的历史发布测试记录。

## 交互

剧集完整整理成功且 rename 消息段已封存后，Host 另发一张「请选择下一步」卡；电影直接结束：
