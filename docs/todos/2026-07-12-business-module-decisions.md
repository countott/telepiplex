...
- download 外部传输中断后不自动重复提交，等待人工确认或显式重试。
- rename 发生破坏性操作中断后不自动重放，保留现场并交由人工恢复。
...
```text
Telegram 搜索请求
→ search 收集证据并生成候选下载计划
→ 用户确认 canonical contract
→ search 调用 download.provider
→ download 提交并跟踪下载
→ download 发布 download.completed
→ rename 获取真实文件树并执行规则优先 mapping
→ AI 只在规则无法确定时提供受 contract 约束的映射候选
→ rename 移动目标视频并清理不保留内容
→ rename 发布 media.organized
→ sync 按最终路径扫描、定位和处理 Plex 条目
...
| sync | 扫描、最终路径定位、Plex 异常处理 | 重做下载计划或文件 mapping |
...
### TODO-03 download 纯化为下载与存储（已实现）

- download 不再为单文件套目录，不再接受或执行业务顶层命名。
...
- rename 先调用 `media.search.resolve_metadata`，由 search 统一重新查询 Wikipedia、豆瓣、TVDB，并在严格证据不足时调用 AI。
...
- 已实现：每个 Feature 默认选择兼容当前 Host API 的最新稳定版本；预发布、不兼容和无效发布不会进入列表。
